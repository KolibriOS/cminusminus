...
int MakeObj() {
  ...
  *(short *)&string2[0] = 0xE940;
  ...
  obj_outrecord(
      0x96, 39,
      (unsigned char
           *)"\000\005_TEXT\004CODE\004_BSS\003BSS\006DGROUP\005_DATA\004DATA");
  // output the SEGDEF
  if (!am32) {
    string2[0] = (unsigned char)0x28;
    *(short *)&string2[1] =
        (short)outptr; // count;// Set the length of the segment of DATA or CODE
    ...
    *(short *)&string2[4] = 0x0103; //класс CODE Overlay NONE 1
    ...
    string2[0] = (unsigned char)0x48;
    *(short *)&string2[1] =
        (short)postsize;            // Set the length of the segment BSS
    ...
    *(short *)&string2[4] = 0x0105; //класс BSS Overlay NONE
    ...
    if (comfile == file_exe && modelmem == SMALL) {
      ...
      string2[0] = 0x74;
      *(short *)&string2[1] =
          (short)stacksize;           // Set the length of the segment STACK
    ...
    *(long *)&string2[1] =
        (long)outptr;  // count;// Set the length of the segment of DATA or CODE
    ...
    *(short *)&string2[6] = 0x0103; //класс CODE Overlay NONE
    ...
    /*
                    ...
                    *(short *)&string2[6]=0x0108;	//класс DATA Overlay
       NONE obj_outrecord(0x99,8,string2); i++;*/
    ...
    *(short *)&string2[6] = 0x0105;        //класс BSS Overlay NONE
    ...
    if (comfile != file_w32) {
      string2[0] = 0x75;
      *(long *)&string2[1] =
          (long)stacksize;            // Set the length of the segment STACK
  ...
  restart:
    for (j = 0; j < posts; j++) {
      if ((postbuf + j)->type >= CALL_EXT &&
          (postbuf + j)->type <= FIX_CODE32 &&
          (postbuf + j)->loc >= (i + startptr) &&
          (postbuf + j)->loc < (i + sizeblock + startptr)) {
        if ((postbuf + j)->loc >
            (i + startptr + sizeblock - (am32 == FALSE ? 2 : 4))) {
          sizeblock = (postbuf + j)->loc - i - startptr; //изменить размер блока
          goto restart;
        }
      }
    }
    if ((i + sizeblock) > count)
      sizeblock = count - i;
    obj_outLEDATA(1, i + startptr, sizeblock, data + i);
    int ofsfix = 0;
    for (j = 0; j < posts; j++) {
      if ((postbuf + j)->loc >= (i + startptr) &&
          (postbuf + j)->loc < (i + sizeblock + startptr)) {
        int hold = (postbuf + j)->loc - i - startptr;
        if ((postbuf + j)->type >= CALL_32I /*POST_VAR*/ &&
            (postbuf + j)->type <= FIX_CODE32) {
          bufobj[ofsfix++] =
              (unsigned char)((am32 == FALSE ? 0xC4 : 0xE4) | (hold / 256));
          ...
          case FIX_VAR32:
            bufobj[ofsfix++] =
                (unsigned char)((comfile == file_exe && modelmem == SMALL) ? 2
                                                                           : 1);
            break;
          case FIX_CODE:
          case FIX_CODE32:
        ...
        if ((postbuf + j)->type >= POST_VAR32 &&
          ...
          if (numext != 0) {
            bufobj[ofsfix++] =
                (unsigned char)((am32 == FALSE ? 0x84 : 0xA4) | (hold / 256));
            bufobj[ofsfix++] = (unsigned char)(hold % 256);
            bufobj[ofsfix++] = 0x56;
    ...
    if (ofsfix != 0)
      obj_outrecord(0x9C + am32, ofsfix, (unsigned char *)bufobj);
    i += sizeblock;
  }
  free(bufobj);
  if (comfile == file_exe && modelmem == SMALL) {
    i = 0;
    while (i < outptrdata) {
      if ((i + 1024) > outptrdata)
        obj_outLEDATA(2, i, outptrdata - i, outputdata + i);
      else
        obj_outLEDATA(2, i, 1024, outputdata + i);
      i += 1024;
    }
  }
  if (sobj != FALSE) {
    ...
    count = EntryPoint();
    *(short *)&string2[0] =
        0xC1; //главный модуль имеет стартовый адрес котор нельзя менять.
...
void obj_outLEDATA(unsigned int segm, unsigned int offset,
  ...
  for (i--; i >= 0; i--)
    checksum += buf[i];
  for (i = 0; (unsigned int)i < recordlength; i++)
    checksum += data[i];
...
void outeachPUBDEF(idrec *ptr) {
  ...
  if (ptr != NULL) {
      ...
      for (unsigned int j = 0; j < posts; j++) { //поиск использования процедуры
        if ((postbuf + j)->num == (unsigned long)ptr->recnumber &&
          ...
          if (externnum >= maxnumextname) {
            maxnumextname += MAXNUMEXTNAME;
            numextname =
                (int *)REALLOC(numextname, maxnumextname * sizeof(int));
          }
        }
      }
      if (i)
    ...
    if (externnum != 0 && ptr->rectok == tk_undefproc &&
        (ptr->flag & f_extern) != 0) {
    nameext:
      ...
      if ((ptr->rectok == tk_proc || ptr->rectok == tk_interruptproc) &&
        ...
        string2[1] = 0x01;
      } else if ((ptr->rectok >= tk_bits && ptr->rectok <= tk_doublevar) ||
        ...
        goto endp;
      for (i = 0; ptr->recid[i] != 0; i++)
        string2[i + 3] = ptr->recid[i];
      string2[2] = (unsigned char)i;
