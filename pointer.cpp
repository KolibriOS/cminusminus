...
void dopointerproc() {
  ...
  if ((FixUp)
      ...
      && ((itok.rm == rm_d32 && am32) ||
          (itok.rm == rm_d16 && am32 == 0))) // 16.07.04 16:01
  ...
  if (wtok.npointr < 2) {
    ...
    outseg(&wtok, 2);
    op(0xFF);
    op(0x10 + wtok.rm + ((wtok.flag & f_far) != 0 ? 8 : 0));
    outaddress(&wtok);
  } else {
    int reg = BX;
    getpointeradr(&wtok, wbuf, &wstr, wtok.npointr - 1, am32 == 0 ? r16 : r32,
                  reg);
    snum = initparamproc();
    op66(razr);
    op(0xFF);
  ...
  if ((wtok.flag & f_typeproc) == tp_cdecl)
    CorrectStack(snum);
  else
    addESP = oaddESP;
  clearregstat(0);
