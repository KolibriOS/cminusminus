...
extern unsigned int
    postnumflag; //флаг последнего идентификатора в вычислении номера
extern unsigned char
    sdp_mode; //режим принудительной выгрузки динамических процедур
...
--------------------------------------------------*/
void CheckAllMassiv(char *&buf, int sizeel, SINFO *strc, ITOK *ctok = &itok,
                    int reg1 = idxregs[0], int reg2 = idxregs[1]);
...
int CheckMassiv(char *&buf, int sizeel, int treg, int *idx = 0, int *base = 0,
                long *num = 0);
void AutoDestructor();
void dodelete();
void donew();
...
int do_d_wordvar(int sign, int razr,
                 int terminater = tk_semicolon); /* signed or unsigned 16 bit
                                                    memory variable */
...
void doregmath_32(int reg, int razr, int sign, char **ofsstr,
                  int i = 0); /* math done is on all regs except AX */
int getintobeg(int beg, char **ofsstr);
int getintoreg_32(
    int reg, int razr, int sign, char **ofsstr,
    int useloop = TRUE); /* get into word reg (except AX) with enum */
void outaddress(ITOK *outtok);
void FloatToNumer(int addop = 0);
int dofloatvar(int addop = 0, int retrez = tk_floatvar,
               int terminater = tk_semicolon);
void fwait3();
...
--------------------------------------------------*/
localrec *addlocalvar(char *str, int tok, unsigned int num,
                      int addmain = FALSE);
int addpoststring(int segm = CS, int len = itok.number,
                  int term = itok.flag); /* add a string to the post queue */
...
void dopointer();
void dovalpointer();
void getpointeradr(ITOK *gstok, char *&gbuf, SINFO *gstr, int numpointer,
                   int razr, int reg = BX);
void dopointerproc();
