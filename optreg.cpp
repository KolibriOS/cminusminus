...
enum { razr_8l, razr_8h, razr_16, razr_32 };
...
#else
  bakregstat =
      (REGISTERSTAT *)MALLOC(sizeof(REGISTERSTAT) * 8 + sizeof(BLVIC *));
...
void AddRegVar(int reg, int razr, ITOK *itok4) {
  if (optinitreg == FALSE || (itok4->flag & f_useidx) != 0)
    return;
  REGEQVAR *cur, *news;
  cur = (REGEQVAR *)MALLOC(sizeof(REGEQVAR));
  ...
  if ((regstat + reg)->next == NULL)
    (regstat + reg)->next = cur;
  else {
...
#endif
  for (int i = 0; i < 8; i++) {
    if ((regstat + i)->type == t_undef ||
        (regstat + i)->type != (bak + i)->type ||
        ((regstat + i)->type == t_id &&
         strcmp((regstat + i)->id, (bak + i)->id) != 0))
      ClearRegBak(i, bak);
  }
}
 ...
 */
{
  ...
  for (i = 0; i < 8; i++) {
    ...
    while (cur) {
      if (cur->razr == nr ||
          (razr == r8 && (cur->razr == razr_8l || cur->razr == razr_8h))) {
  ...
  if (retreg != NOINREG) {
    if (razr == r8) {
      if ((regstat + retreg)->razr == razr_8h)
        retreg += 4;
      if (nr == razr_8h)
        reg += 4;
      waralreadinitreg(begs[reg], begs[retreg]);
    } else
      waralreadinitreg(regs[razr / 4][reg], regs[razr / 4][retreg]);
  }
  return retreg;
}
...
{
  int nr;
  if (optinitreg == FALSE || name == NULL || razr > r32)
    return;
  nr = convertrazr(&reg, razr);
  ClearReg(reg);
...
void GetEndLex(int stop1, int stop2, int type) {
  ...
  scanlexmode = DEFLEX;
  while (tok2 != stop1 && tok2 != stop2 && tok2 != tk_eof &&
         itok2.type != type) {
    nexttok();
    while (tok == tk_closebracket && i) {
      i--;
...
int RegToReg(int regd, int regs, int razr) {
  ...
  if (razr == r8 && nrs != nr && regs == regd)
    goto noreg;
  if (nrs == nr && (regstat + regd)->type == (regstat + regs)->type &&
      (regstat + regs)->type != t_undef &&
      (regstat + regd)->razr == (regstat + regs)->razr) {
    if (strcmp((regstat + regd)->id, (regstat + regs)->id) == 0)
      return SKIPREG;
  }
noreg:
  ClearReg(regd);
...
int RegSwapReg(int reg1, int reg2, int razr) {
  ...
  if ((regstat + reg1)->type && (regstat + reg1)->razr > nr)
    ClearReg(reg1);
  if ((regstat + reg2)->type && (regstat + reg2)->razr > nrs)
    ClearReg(reg2);
  if ((regstat + reg1)->razr < razr_16 && nrs >= razr_16)
    ClearReg(reg1);
  if ((regstat + reg2)->razr < razr_16 && nr >= razr_16)
    ClearReg(reg2);
  if (razr == r8 && nrs != nr && reg1 == reg2)
    goto noreg;
  if ((regstat + reg1)->type == (regstat + reg2)->type &&
      (regstat + reg2)->type != r_undef &&
      (regstat + reg1)->razr == (regstat + reg2)->razr) {
    if (strcmp((regstat + reg1)->id, (regstat + reg2)->id) == 0)
      return SKIPREG;
  }
noreg:
  KillRegLec(reg1);
...
void CopyLVIC(BLVIC *bak) {
  ...
  if (cursizevic > bak->sizevic) {
    for (int i = bak->sizevic; i < cursizevic; i++)
      (listvic + i)->rec = NULL;
  } else
    cursizevic = bak->sizevic;
}
    ...
    if ((bak->listvic + i)->rec != NULL) {
      ...
      for (j = 0; j < cursizevic; j++) {
        if ((listvic + j)->rec == bvic->rec) {
          if ((listvic + j)->lnumber != bvic->lnumber)
            bvic->rec = NULL;
          break;
        }
      }
...
int Const2Var(ITOK *itok4, long long num, int typenum) {
  ...
  if (replasevar && itok4->rec && (itok4->flag & f_useidx) == 0) {
    if ((typenum == tk_word || typenum == tk_dword) && num < 256)
      typenum = tk_byte;
    for (i = 0; i < cursizevic; i++) {
      if ((listvic + i)->rec == itok4->rec)
        break;
      if (freevic == -1 && (listvic + i)->rec == NULL)
        freevic = i;
    }
    if (i == cursizevic) {
      if (freevic == -1) {
        ...
        for (freevic = cursizevic - 1; freevic > i; freevic--)
          (listvic + freevic)->rec = NULL;
      } else
        i = freevic;
    } else if ((listvic + i)->contype == typenum &&
...
void ClearVarByNum(ITOK *itok4) {
  if (replasevar && itok4->rec && (itok4->flag & f_useidx) == 0) {
    for (int i = 0; i < cursizevic; i++) {
      if ((listvic + i)->rec == itok4->rec) {
        (listvic + i)->rec = NULL;
        /*				printf("%s(%d)> Clear by constant var
           '%s'\n",
                                                startfileinfo==NULL?"":(startfileinfo+currentfileinfo)->filename,
                                                linenumber,
                                                itok4->name);*/
...
int UpdVarConst(ITOK *itok4, long long num, int typenum, int operand) {
  int retcode = FALSE;
  int i;
  if (replasevar == 0 || itok4->rec == NULL || (itok4->flag & f_useidx) != 0)
    return TRUE;
  for (i = 0; i < cursizevic; i++) {
    if ((listvic + i)->rec == itok4->rec) {
      //			printf("%s(%d)> Modif var '%s' by num=%u
      //",startfileinfo==NULL?"":(startfileinfo+currentfileinfo)->filename,linenumber,itok4->name,(listvic+i)->number);
      switch (operand) {
      case tk_plusequals:
      case tk_plusplus:
        operand = tk_plus;
      ...
      case tk_llequals:
        ...
        break;
      case tk_numsign:
        if ((listvic + i)->contype == tk_float)
          (listvic + i)->number |= 0x80000000;
      ...
      case tk_word:
      case tk_dword:
        retcode = calcdwordnumber((unsigned long *)&(listvic + i)->number, num,
                                  operand);
        break;
      case tk_qword:
        retcode = calcqwordnumber((unsigned long long *)&(listvic + i)->lnumber,
                                  num, operand);
        break;
      case tk_float:
        retcode =
            calcfloatnumber(&(listvic + i)->fnumber, *(float *)&num, operand);
        break;
      case tk_double:
        retcode =
            calcdoublenumber(&(listvic + i)->dnumber, *(double *)&num, operand);
        break;
      }
      /*			if(retcode)printf("new
         num=%u\n",(listvic+i)->number); else{ puts("error");
                                      (listvic+i)->rec=NULL;
                              }                       */
      break;
    }
...
int CheckRegToConst(int reg, ITOK *itok4, int razr) {
  ...
  int nr;
  nr = convertrazr(&reg, razr);
  if (replasevar && optinitreg != FALSE && razr <= r32 &&
...
int CheckUpdRegToConst(int reg, ITOK *itok4, int operand, int razr) {
  ...
  if (replasevar && optinitreg != FALSE && razr <= r32 &&
    ...
    if (isdigit((regstat + reg)->id[0])) {
      if (getnumber((regstat + reg)->id, &num, &typenum))
        return UpdVarConst(itok4, num, typenum, operand);
    }
  }
  return FALSE;
}
...
int SwapVarConst(ITOK *itok2, ITOK *itok4) {
  ...
  if (replasevar == 0)
    return TRUE;
  if (itok4->rec == NULL || (itok4->flag & f_useidx) != 0)
    sec = -2;
  ...
  for (i = 0; i < cursizevic; i++) {
    if (sec == -1 && (listvic + i)->rec == itok4->rec)
      sec = i;
    else if (fir == -1 && (listvic + i)->rec == itok2->rec)
      fir = i;
  }
  if (sec == -2)
    sec = -1;
  ...
  if (sec != -1 && fir != -1) {
    ...
    (listvic + fir)->rec = itok4->rec;
  else if (fir == -1 && sec != -1)
    (listvic + sec)->rec = itok2->rec;
  return TRUE;
}
  ...
  if (replasevar && itok4->rec && (itok4->flag & f_useidx) == 0 &&
    ...
    if (isdigit((regstat + reg)->id[0])) {
      if (getnumber((regstat + reg)->id, &num, &typenum)) { //цифра в регистре
        if (i != cursizevic)
          ConstToReg((listvic + i)->lnumber, reg, razr); //новую в регистр
        else {
          if (freevic == -1) {
            cursizevic += MAXSIZEVIC;
            listvic = (LVIC *)REALLOC(listvic, sizeof(LVIC) * cursizevic);
...
int CheckConstVar(ITOK *itok4) {
  if (replasevar == FALSE || itok4->rec == NULL ||
      (itok4->flag & f_useidx) != 0)
    return FALSE;
...
void CheckConstVar3(int *tok4, ITOK *itok4, int razr) {
  ...
  if (*tok4 >= tk_charvar && *tok4 <= tk_doublevar) {
    if ((optimizespeed || itok4->type == tp_gvar || itok4->type == tp_postvar ||
         itok4->number > 127 || (int)(itok4->number) < -127) &&
        CheckConstVar(itok4)) {
      switch (*tok4) {
      case tk_charvar:
      case tk_bytevar:
