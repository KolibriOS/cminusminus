...
void r_Dialog() {
  ...
  while (!CloseBlock() && tok != tk_eof) {
    ...
    *(unsigned long *)&resbuf[curposbuf] = defdialog[restok].style | 0x50000000;
    ...
    case rc_scrollbar:
      if (exts)
        *(unsigned int *)&resbuf[curposbuf + 12] = (unsigned int)GetNumber(i);
      else
        *(unsigned short *)&resbuf[curposbuf + 16] =
            (unsigned short)GetNumber(i);
      expecting(tk_camma);
      while (tok == tk_endline)
        nexttok();
      ...
      if (tok != tk_endline) {
        *(unsigned long *)&resbuf[curposbuf] |= (unsigned long)GetNumber(i + 5);
        if (tok == tk_camma) {
          nexttok();
          *(unsigned long *)&resbuf[curposbuf + (exts == TRUE ? -4 : 4)] =
              (unsigned long)GetNumber(i + 6);
        }
      }
      if (exts && tok == tk_number)
        *(unsigned long *)&resbuf[curposbuf - 4] = GetNumber(i + 9);
      while (tok != tk_endline && tok != tk_eof && tok != tk_rescommand)
        nexttok(); //пропуск излишних параметров
      break;
    case rc_icon:
      ...
      while (tok == tk_endline)
        nexttok();
      *(unsigned short *)&resbuf[curposbuf + (exts == TRUE ? 4 : 8)] =
          (unsigned short)GetNumber(3);
      expecting(tk_camma);
      while (tok == tk_endline)
        nexttok();
      *(unsigned short *)&resbuf[curposbuf + (exts == TRUE ? 6 : 10)] =
          (unsigned short)GetNumber(4);
      if (tok == tk_camma) {
        nexttok();
        *(unsigned short *)&resbuf[curposbuf + (exts == TRUE ? 8 : 12)] =
            (unsigned short)GetNumber(5);
        if (tok == tk_camma) {
          nexttok();
          *(unsigned short *)&resbuf[curposbuf + (exts == TRUE ? 10 : 14)] =
              (unsigned short)GetNumber(6);
          if (tok == tk_camma) {
            nexttok();
    ...
    while (tok == tk_endline)
      nexttok();
    *(unsigned short *)&resbuf[poscount] =
        (unsigned short)(*(unsigned short *)&resbuf[poscount] + 1);
...
unsigned char *LoadBitmap() {
  ...
  if (name[0] != 0)
    ...
    do {
      ...
      do {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          if (isdigit(cha))
            hold += (unsigned char)(cha - '0');
          else if (isxdigit(cha))
            hold += (unsigned char)((cha & 0x5f) - '7');
          else
    ...
    bitobr = LoadFileBin((char *)string);
  return bitobr;
}
...
void NewResourse() {
  ...
  if (resname[0] == 0)
    ...
    curtres->tname = BackString(resname);
  if (idname[0] == 0)
    curtres->id = idnum;
  else
    curtres->name = BackString(idname);
...
void r_Icon() {
  ...
  unsigned int i;
  for (i = 0; i < 6; i++)
    icohead[i] = icoobr[i]; //заголовок
  unsigned int ofs = 6;
  unsigned int ofs2 = 6;
  for (i = 0; i < countico; i++) {
    int j;
    for (j = 0; j < 12; j++)
      icohead[j + ofs] = icoobr[j + ofs2]; //описание иконки
    iconcount++;
    *(unsigned short *)&icohead[ofs + 12] =
        (unsigned short)iconcount; //ее номер
    ...
    for (j = 0; (unsigned int)j < size; j++)
      curtres->res[j] = icoobr[j + ofs3];
    ofs += sizeof(_RESDIR_);
...
void r_Cursor() {
  ...
  unsigned int countcur =
      *(unsigned short *)&curobr[4]; //число курсоров в файле
  ...
  for (i = 0; i < countcur; i++) {
    ...
    *(unsigned short *)&curhead[ofs + 12] =
        (unsigned short)cursorcount; //ее номер
    GetResBlock();                   //образ курсора
    curtres->type = CRT_CURSOR;
    curtres->id = cursorcount;
    curtres->size = size = *(unsigned long *)&curhead[ofs + 8] =
        *(unsigned long *)&curobr[ofs2 + 8] + 4;
    curtres->res = (unsigned char *)MALLOC(size);
    AddType(CRT_CURSOR);
    ...
    *(unsigned short *)&curtres->res[2] = *(unsigned short *)&curobr[ofs2 + 6];
    size -= 4;
    for (int j = 0; (unsigned int)j < size; j++)
      curtres->res[j + 4] = curobr[j + ofs3];
    ofs += sizeof(_CURDIR_);
    ...
    do {
      ...
      if (tok == tk_string) {
        ...
        if (c == '^') {
          c = string[1];
          if (c < 0x40)
            preerror("Unsolved symbol for Contrl"); // error
          c -= 0x40;
        }
      ...
      *(unsigned short *)&resbuf[curposbuf + 4] = (unsigned short)GetNumber(2);
      if (tok == tk_camma) {
        nexttok();
        if (tok == tk_number)
          *(unsigned short *)&resbuf[curposbuf] |= (unsigned short)itok.number;
        *(unsigned short *)&resbuf[curposbuf] |=
            GetFlag((_STRINGS_ *)&typeacceler, 5); // GetAccerFlag();
        nexttok();
        while (tok != tk_endline) {
          if (tok == tk_camma)
            nexttok();
      ...
      if ((tuse + i)->id == (listres + j)->type) {
        ...
        if ((tuse + i)->count == 1) {
          ...
          if (k == 0)
            ...
            do {
              m--;
              if ((listres + j)->name == NULL &&
                  (listres + sortidx[m + n])->name == NULL) {
                if ((listres + j)->id >=
                    (listres + sortidx[m + n])->id) { //новый больше
                  if ((listres + j)->id ==
                      (listres + sortidx[m + n])->id) { //равны
                    numidres--;
                    numlangres++;
                    if ((listres + j)->lang == 0) {
                      numlangres++;
                ...
                int cmp;
                if ((cmp = strcmp((listres + j)->name,
                                  (listres + sortidx[m + n])->name)) >= 0) {
                  if (cmp == 0) {
                    numidres--;
                    numlangres++;
                    if ((listres + j)->lang == 0) {
              ...
              if (m == 0)
                sortidx[sortpos - k] = (unsigned short)j;
            } while (m > 0);
            sortpos++;
          }
        }
  ...
  *(unsigned short *)&resbuf[14] = (unsigned short)(numtyperes - numzerotype);
  ...
  for (i = 0; i < numtyperes; i++) {
    if ((tuse + i)->id == 0) {
      ...
      *(unsigned long *)&resbuf[curposbuf] = (tuse + i)->id;
    ...
    while ((unsigned int)curres->type == type) {
      ...
      if (curres->name) { //добавить имя
        if (j < (numres - 1) &&
          ...
          while (strcmp(curres->name, (listres + sortidx[j + 1])->name) == 0)
            j++;
        *(unsigned short *)&resbuf[ofsback] =
            (unsigned short)(*(unsigned short *)&resbuf[ofsback] + 1);
        *(unsigned long *)&resbuf[curposbuf] = startdata | 0x80000000;
        ...
        *(unsigned short *)&resbuf[startdata] = (unsigned short)len;
        ...
        if (j < (numres - 1) &&
            type == (unsigned int)(listres + sortidx[j + 1])->type)
          while (curres->id == (listres + sortidx[j + 1])->id)
            j++;
        *(unsigned short *)&resbuf[ofsback + 2] =
            (unsigned short)(*(unsigned short *)&resbuf[ofsback + 2] + 1);
      ...
      if (j != k) { //несколько имен с разными языками
        ...
        *(unsigned long *)&resbuf[startlang + 12] = j - k + 1;
        ...
        if (curres->lang) { //указан язык
          ...
          *(unsigned long *)&resbuf[curposbuf] = startofsdata;
        startofsdata += 16;
      }
      curposbuf += 4;
      j++;
  ...
  for (i = 0; i < numres; i++) {
    ...
    *(unsigned long *)&resbuf[curposbuf] = startdata + ofsres;
    if (!numrel)
      listr = (LISTRELOC *)MALLOC(sizeof(LISTRELOC));
    else
      listr = (LISTRELOC *)REALLOC(listr, (numrel + 1) * sizeof(LISTRELOC));
