...
char id2[ID2S][9] = {
    "ESCHAR",  "ESBYTE",  "ESINT",    "ESWORD",  "ESLONG",  "ESDWORD",
    "ESFLOAT", "ESQWORD", "ESDOUBLE", "CSCHAR",  "CSBYTE",  "CSINT",
    "CSWORD",  "CSLONG",  "CSDWORD",  "CSFLOAT", "CSQWORD", "CSDOUBLE",
    "SSCHAR",  "SSBYTE",  "SSINT",    "SSWORD",  "SSLONG",  "SSDWORD",
    "SSFLOAT", "SSQWORD", "SSDOUBLE", "DSCHAR",  "DSBYTE",  "DSINT",
    "DSWORD",  "DSLONG",  "DSDWORD",  "DSFLOAT", "DSQWORD", "DSDOUBLE",
    "FSCHAR",  "FSBYTE",  "FSINT",    "FSWORD",  "FSLONG",  "FSDWORD",
    "FSFLOAT", "FSQWORD", "FSDOUBLE", "GSCHAR",  "GSBYTE",  "GSINT",
...
unsigned char cha2;
char skipfind =
    FALSE; /* пропуск поиска в глобальном дереве и среди
  ...
  loopsw:
    ...
    case tk_postnumber:
      if (dsword == 0 || sizevar != 1)
        goto runblock;
      if ((cstok.post & USED_DIN_VAR) ==
          USED_DIN_VAR) { //динамическая переменная
        if (dstok.rec != NULL)
          goto runblock;
        dstok.rec = cstok.rec;
      }
    ...
    runblock:
      ...
      if (bufrm) {
        if (strcmp(bufrm, "&this;") == 0) {
          free(bufrm);
          CharToBackBuf(0);
          sprintf((char *)string3, "&%s*%d+this;", BackTextBlock,
                  GetVarSize(ttok));
...
void CheckAllMassiv(char *&buf, int sizeel, SINFO *strc, ITOK *ctok, int reg1,
                    int reg2) {
...
int CheckMassiv(char *&buf, int sizeel, int treg, int *idx, int *base,
                long *num) {
  ITOK oitok;
  SINFO ostr;
  ...
  if (am32 && sizeel == 1 && tok == tk_reg32 && itok2.type == tp_stopper)
    ...
    if ((chip == 7 || chip == 8) && am32 == FALSE &&
        ((sizeel > 1 && sizeel < 6) || sizeel == 8 || sizeel == 9)) {
...
void nexttok() {
  ...
  if (tok == tk_dollar) {
    if (itok2.type == tp_opperand || itok2.type == tp_stopper ||
        tok2 == tk_dollar) {
      tok = tk_number;
      itok.number = outptr;
      if (FixUp)
...
#endif
  if (usedirectiv == FALSE && calcnumber == FALSE)
...
void GetTokString(int *tok4, ITOK *itok4, unsigned char *string4,
                  int useunicode) {
  int strptr = 0;
  if (displaytokerrors) {
    savestring3 = TRUE;
...
nextstr:
  ...
  while (cha != '\"' && !endoffile && strptr < STRLEN - 1) {
    ...
    if ((char)cha == 'n' &&
        string4[strptr - 1] == 13) { // have to add char 10 for \n value
      string4[strptr++] = 10;
    }
    nextchar();
  }
...
#ifdef __CONSOLE__
    strptr = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, bak, -1,
                                 (wchar_t *)string4, STRLEN) -
             1;
#else
...
//		if(displaytokerrors)printf("%s (%u)
//>%s\n",(startfileinfo+currentfileinfo)->filename,linenumber,itok4->name);
        ...
        if (string4[1] == 'S' || (asmparam && string4[1] == 's')) {
          for (useme = 0; useme < 6; useme++) { // check for CS, SS, ...
            if (string4[0] == segs[useme][0] ||
                (asmparam && (string4[0] & 0x5F) == segs[useme][0])) {
              if (cha == ':') {
                int oinptr = inptr;
    ...
    if (strptr == 3 &&
        (string4[0] == 'E' ||
         (asmparam && string4[0] == 'e'))) { // check for EAX, ECX, EDX, ...
      for (useme = 0; useme < 8; useme++) {
        int i;
        if (asmparam)
    ...
    if (asmparam) {
      ...
      } else if ((*(short *)&string[1] & 0x5f5f) == 0x4d4d &&
                 (string4[0] & 0x5f) == 'X') {
    ...
    if (*tok4 == tk_id || *tok4 == tk_ID) {
      if (cur_mod && cur_mod->numparamdef &&
          (displaytokerrors || scanalltoks)) { // 18.03.05 15:12
        /*				if(debug&&cur_mod){
                                                printf("Find %s in %d entries
           curmod=%08X\n",itok4->name,cur_mod->numparamdef,cur_mod); COM_MOD
           *nmod; nmod=cur_mod; while(nmod){ printf("\tinput=%08X
           inptr=%08X\n",nmod->input,nmod->inptr); nmod=nmod->next;
                                                }
                                        }*/
        if (CheckDefPar(itok4->name)) {
          ...
          if (debug)
            printf(
                "1601 tok=%d input=%08X inptr=%08X disp=%d %s %c%40s\n\t%s\n",
                *tok4, input, inptr, displaytokerrors, itok4->name, cha,
                input + inptr, "" /*input*/);
#endif
          return;
        }
      }
      if (searchlocals(itok4, tok4, string4) ==
          FALSE) { //поиск среди локальных меток
        ...
        if (*tok4 == tk_endline) {
          if (scanlexmode != DEFLEX) {
            ...
            if (debug)
              printf(
                  "1610 tok=%d input=%08X inptr=%08X disp=%d %s %c%40s\n\t%s\n",
                  *tok4, input, inptr, displaytokerrors, itok4->name, cha,
                  input + inptr, "" /*input*/);
#endif
            return;
        ...
        if ((*tok4 >= tk_bits && *tok4 <= tk_doublevar) ||
          ...
          if (itok4->post ==
              DYNAMIC_POST) { //преобразовать динамическую локальную в локальную
            if (alignword && *tok4 != tk_charvar &&
                *tok4 != tk_bytevar) { //выровнять на четный адрес
              switch (*tok4) {
              case tk_intvar:
              case tk_wordvar:
                if (postsize % 2 == 1)
    ...
    case tk_id:
      if (findofset == FALSE) {
        ...
        if (((tteg = FindTeg(FALSE, itok4->name)) != NULL ||
          ...
          //						printf("tok=%d\n",*tok4);
          if (displaytokerrors &&
              (*tok4 == tk_proc || *tok4 == tk_declare ||
               *tok4 == tk_undefproc) &&
              (itok4->flag & f_static) == 0)
  ...
  yesid:
    ...
    if (itok4->rm != rm_d16 && *tok4 != tk_proc && *tok4 != tk_undefproc &&
        *tok4 != tk_apiproc && *tok4 != tk_declare &&
        (!(*tok4 == tk_pointer && itok4->type == tk_proc)))
      if (itok4->post == 0 &&
          (itok4->flag & f_reloc) == 0) { // cannot compress if POST var
  ...
  if (debug)
    printf("2172 tok=%d input=%08X inptr=%08X disp=%d %s %X %s\n", *tok4, input,
           inptr, displaytokerrors, itok4->name, cha, input + inptr);
#endif
}
...
//поиск в дереве переменых
{
  ...
  for (ptr = fptr; ptr != NULL;) {
    if ((cmpresult = strcmp(ptr->recid, (char *)string4)) == 0) {
      if (scanlexmode == RESLEX && ptr->rectok != tk_number &&
          ptr->rectok != tk_string)
        break;
        ...
        default:
          ...
          if (strcmp((char *)string4, ptr->recid) !=
            ...
            case tk_undefproc:
              ...
              break;
            default:
              strncpy(itok4->name, (char *)string4,
                      IDLENGTH - 1); //имя нужно для undefine
              break;
            }
            return TRUE;
          }
...
void AddDynamicList(idrec *ptr) {
  ...
  if (DynamicList == NULL) {
    ...
    sizeDL += STEPDL;
    DynamicList =
        (volatile idrec **)REALLOC(DynamicList, sizeof(idrec **) * sizeDL);
  }
  for (int i = 0; i < countDP; i++)
    if (ptr == DynamicList[i])
      return;
...
void Destructor(idrec *ptrs) {
  ...
  if ((bazael = FindClassEl(tteg, (unsigned char *)name, &addofs, NULL)) ==
      NULL) {
  ...
  for (ftlr = tlr; ftlr != NULL; ftlr = ftlr->next) {
    for (ptrs = ftlr->lrec; ptrs != NULL; ptrs = ptrs->rec.next) {
      if (ptrs->rec.rectok == tk_structvar) {
        if ((ptrs->rec.flag & fs_destructor) != 0 ||
            AskUseDestr((structteg *)ptrs->rec.newid)) {
          if (!calldestruct) {
            calldestruct = TRUE;
  ...
  for (unsigned int i = 0; i < searcht->numoper; i++) {
    if ((bazael + i)->tok == tk_baseclass) {
      if (!numclass)
        ...
        listclass =
            (void **)REALLOC(listclass, sizeof(void **) * (numclass + 1));
    ...
    for (ptr = ftlr->lrec; ptr != NULL; ptr = ptr->rec.next) {
      ...
      if (strcmp(ptr->rec.recid, (char *)string4) == 0) {
        ...
        *tok4 = ptr->rec.rectok;
        ...
        } else if (!(ptr->rec.rectok == tk_locallabel ||
                     ptr->rec.rectok == tk_number)) {
        ...
        *tok4 = tk_structvar;
        ...
        if (*tok4 == tk_structvar) {
          ...
          itok4->number +=
              (itok4->post == 0
                   ? 0
                   : itok4->rm * ((structteg *)bazael->nteg)->size);
          itok4->size = (itok4->post == 0 ? bazael->numel : 1) *
                        ((structteg *)bazael->nteg)->size;
    ...
    unsigned char *string4) //разбор структур на переменные и структуры
{
  ...
  if (cha == '.') {
    ...
    if (fdestr)
      strcat(cstok.name, "~");
    if ((bazael = FindClassEl(tteg, (unsigned char *)cstok.name, &addofs,
                              subteg)) == NULL) {
      if (displaytokerrors)
    ...
    *tok4 = bazael->tok;
    ...
    if (*tok4 == tk_proc || *tok4 == tk_pointer) {
      ...
      if (*tok4 == tk_proc) {
        ...
        /*				if(strcmp(bazael->name,tteg->name)==0&&itok4->rec){
        //вызов конструктора ptr=itok4->rec;
        //					printf("constructor
        %08X\n",ptr);
                                        }*/
      }
      //			if(ptr)printf("new address constructor
      //%08X\n",bazael->rec);
      ptr = bazael->rec;
      itok4->type = ptr->type;
      itok4->npointr = ptr->npointr;
  ...
  //	itok4->size=(numel==0?bazael->numel:1)*((structteg
  //*)bazael->nteg)->size; 	itok4->rec=(idrec *)tteg;
  itok4->rm = numel;
...
void dosizeof(ITOK *itok4) //опр значение sizeof
{
  ...
  //	if(displaytokerrors)printf("tok=%d findoffset=%d
  ...
  if (strcmp(cstok.name, "__CODESIZE") == 0) {
    ...
    case tk_interruptproc:
      ...
      break;
    default:
      if ((tteg = FindTeg(FALSE, cstok.name)) != NULL ||
          (tteg = FindTeg(TRUE, cstok.name)) != NULL) {
        if (cha == '.') {
...
void ofsstr(int *tok4, ITOK *itok4) {
  ...
  if (poststr == LOCAL) {
    ...
    if (strinf.bufstr == NULL) {
      ...
      if (displaytokerrors) {
        if ((newreg = CheckIDXReg(strinf.bufstr, strinf.size, idxregs[0])) !=
            NOINREG) {
          if (newreg == SKIPREG) {
            newreg = idxregs[0];
          } else {
    ...
    *tok4 = tk_rmnumber;
    ...
    if (strinf.bufstr == NULL) {
      ...
      if (displaytokerrors) {
        ...
        newreg = (idxregs[4] == 255 ? idxregs[0] : idxregs[4]); // 14.06.06
                                                                // 20:06
      noopt2:
        CheckMassiv(strinf.bufstr, strinf.size,
                    (idxregs[4] == 255 ? idxregs[0] : idxregs[4]));
      cont2:;
      }
      itok4->rm =
          RegToRM(newreg, (am32 == FALSE ? tk_reg : tk_reg32)) | rm_mod10;
      *tok4 = tk_rmnumber;
    }
    itok4->segm = DS;
  }
...
void AddUndefOff(
    int segm,
    char *ostring) //зафиксировать обращение к еще не объявленым именам
  ...
  for (curptr = undefoffstart;; curptr = curptr->next) {
    if (strcmp(curptr->name, ostring) == 0) { //ранее уже обращались к ней
      ...
      if (curptr->pos == NULL)
        ...
        curptr->pos =
            (IOFS *)REALLOC(curptr->pos, sizeof(IOFS) * (curptr->num + 1));
  ...
  for (curptr = undefoffstart;; curptr = curptr->next) {
    if (strcmp(curptr->name, (char *)name) == 0) { //нашли
      for (int i = 0; i < curptr->num; i++) {
        ...
        if (base == DS &&
            dynamic_flag) { //было обращение к динамическим иниц. переменным
          CheckPosts();
          (postbuf + posts)->type =
          ...
          if ((segm & 1) == 0 || modelmem == TINY) { //в сегменте кода
            ...
            if (am32 == FALSE)
              *(unsigned short *)&outputdata[ofs] +=
                  (unsigned short)(base == CS ? outptr : outptrdata);
            else
              *(unsigned long *)&outputdata[ofs] +=
                  (unsigned long)(base == CS ? outptr : outptrdata);
          }
          ...
          if (base == VARPOST) {
            ...
            (postbuf + i)->type =
                (unsigned char)(am32 == FALSE ? POST_VAR : POST_VAR32);
          }
        }
      }
      free(curptr->pos);
...
int GetDirective(char *str) {
  ...
  //	printf("i=%d %s\n",i,str);
  if (i != -1 && i < d_end1 && notdoneprestuff != TRUE &&
      displaytokerrors != 0) {
    char buf[80];
...
void AddTegToTree(structteg *newteg, int Global) {
  ...
  if (tteg == NULL)
    ...
    while (((i = strcmp(tteg->name, newteg->name)) < 0 && tteg->left != NULL) ||
           (i > 0 && tteg->right != NULL))
                           ...
                           int noname) //создать новый тег
{
  ...
  while (tok != tk_closebrace && tok != tk_eof) {
    ...
    default:
      ...
      utestInitVar = TRUE;
      if ((i = testInitVar()) == FALSE ||
          i == 2) { //определение процедуры пока не обрабатываем
      ...
      do {
        ...
        if (tok == tk_colon) {
          ...
          if (numt == 0)
            numt =
                size * 8 -
                bitofs % (size * 8); //(am32==0?16:32)-bitofs%(am32==0?16:32);
          bitofs += numt;
          continue;
        }
      dproc2:
        ...
        if (tok != tk_ID && tok != tk_id) {
          utestInitVar = TRUE;
          if (testInitVar() ==
              FALSE) { //определение процедуры пока не обрабатываем
            idalreadydefined();
          } else {
            oflag |= itok.flag;
            npointr = itok.npointr;
            ...
            if (tok2 == tk_openbracket &&
                strcmp(itok.name, newteg->name) == 0) {
              newteg->flag |= fs_constructor;
              oflag |= fs_constructor;
              if (orm == tokens)
                orm = am32 == FALSE ? tk_word : tk_dword;
 ...
 --------------------------------------------------*/
{
  ...
  loopsw:
    ...
    case tk_minus:
    case tk_number:
      if (type == tk_bytevar || type == tk_wordvar || type == tk_dwordvar ||
          type == tk_bits || type == tk_pointer) {
        hold = doconstdwordmath();
      } else if (type == tk_charvar || type == tk_intvar ||
                 type == tk_longvar) {
                 ...
                 structteg *tteg) //инициализировать глобальную структуру
{
  ...
  while (tok != tk_eof && done == 0) {
    ...
    case tk_ID: //инициализировать структуру
      ...
      if (ptr == NULL)
        ((flag & f_static) == 0 ? treestart : staticlist) =
            newrec; //начало дерева
      else {        //поиск строки в дереве
        while (((numel = strcmp(ptr->recid, itok.name)) < 0 &&
                ptr->left != NULL) ||
               (numel > 0 && ptr->right != NULL)) {
      ...
      if (tok == tk_assign || notpost == TRUE) { //=
        ...
        newrec->recnumber =
            (dynamic == 0 ? outptrdata : 0); //адрес начала структуры
        ...
        if (CheckUseAsUndef((unsigned char *)newrec->recid) == 0 &&
            dynamic_flag)
          dynamic = TRUE;
...
void InitStruct() //инициализировать глобальную структуру
{
  ...
  tteg = FindTeg(TRUE);
  if (tteg == NULL) {
    if (tok == tk_openbrace || tok2 == tk_openbrace || tok == tk_colon ||
        tok2 == tk_colon)
...
unsigned long
LocalStruct2(int flag, int *localline, int binptr, char bcha,
...
void dostruct() {
  ...
  if (tok == tk_assign) {
    ...
    case tk_dwordvar:
      ...
      wstr = strinf;
      strinf.bufstr = NULL;
      if ((retreg = CheckIDZReg(itok.name, AX, razr = GetVarSize(tok))) !=
          NOINREG) {
    ...
    case tk_number:
      ...
      if (strinf.bufstr == NULL) {
        if (optimizespeed == TRUE) {
          ...
          if (am32 == FALSE) {
            if (sized == 1 && ((num < 7 && (num % 2) == 0) ||
                               (itok.number == 0 && num == 8))) {
              sized = 2;
              itok.number = (itok.number & 255) | ((itok.number << 8) & 0xFF00);
            }
          } else if (chip > 2 && sized != 4 && (num % 4) == 0 &&
                     itok.number == 0)
            sized = 4;
        }
      }
      ...
      if (num == 1) {
        ...
        oneloop++;
        itok.number &= 255;
        hnum = itok.number | (itok.number << 8) | (itok.number << 16) |
               (itok.number << 24);
        sized = 4;
        num = 1;
      } else {
    ...
    case tk_structvar:
      ...
      if (localstr2) {
        ...
        itok.rm =
            (am32 == FALSE ? rm_d16
                           : rm_d32); // 22.11.04 09:22//установки по умолчанию
        if (strinf.bufstr == NULL) {
          if (itok.post == TRUE)
            tok = tk_postnumber;
          else
      ...
      if (usenumstr == FALSE)
        num *=
            tteg->size; // ptrs->recrm;	//не указан номер структуры - значит все
  ...
  if (isdigit(cha)) { //числа
    ...
    *tok4 = tk_number;
    ...
    case '?':
      ...
      //			if((itok.number=GetDirective((char
      //*)string))!=-1){
