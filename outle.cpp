...
unsigned int EntryParamStr() {
  ...
  if (bb == tk_id)
    ...
    if ((outptrdata % 2) == 1)
      btok.number++; /* alignment of entire post data block manditory */
  }
  return btok.number;
}
...
int MakeLE() {
  ...
  if (fwrite(&ot, sizeof(Object_Table) * 2, 1, hout) != 1)
    goto errwrite;
  ...
  if (fwrite(pmt, sizeb, 1, hout) != 1)
    goto errwrite;
  free(pmt);
  hdr.Resource_Table_Offset = hdr.Resident_Names_Table_Offset = headsize +=
      sizeb;
  ...
  *(long *)&string2[i + 2] = 0;
  if (fwrite(string2, i + 5, 1, hout) != 1)
    goto errwrite;
  ...
  for (i = 0; i < hdr.Number_Of_Memory_Pages; i++) {
    ...
    for (unsigned int j = 0; j < posts; j++) { //обходим всю таблицу post
      if (((postbuf + j)->type == CALL_32I ||
           ((postbuf + j)->type >= POST_VAR32 &&
            (postbuf + j)->type <= FIX_CODE32)) &&
          (postbuf + j)->loc >= startblc &&
          (postbuf + j)->loc < (startblc + 4096)) {
        int sizerec;
        string2[0] = 7;
        *(unsigned short *)&string2[2] =
            (unsigned short)((postbuf + j)->loc - startblc);
        string2[4] = 1;
        ...
        sizefixpage += sizerec;
        if (fwrite(string2, sizerec, 1, hout) != 1)
          goto errwrite;
      }
  ...
  string2[0] = 0;
  if (fwrite(string2, 1, 1, hout) != 1)
    goto errwrite;
  headsize++;

  hdr.Fixup_Section_Size =
      headsize - hdr.Fixup_Page_Table_Offset; // 9+sizefixpage;
  ...
  if (fwrite(fpto, sizeof(unsigned long) * (hdr.Number_Of_Memory_Pages + 1), 1,
             hout) != 1)
    goto errwrite;
  free(fpto);
