...
void AddDataNullLine(char ssize, char *name) {
  ...
  dbgnum[pdbg - 1] = 0; //очистить номер строки
                        // new !!!
  if (name != NULL)
    lststring[pdbg - 1] = BackString(name);
}
...
void AddEndLine() {
  if (pdbg /*&&lstend[pdbg-1]==0*/)
    lstend[pdbg - 1] = outptr;
}

void AddLine(int SkipLineInfo) {
  if (/*ooutptr!=outptr&&*/ dbgact == 0 &&
      (oline != linenumber || omodule != currentfileinfo)) {
    ...
    if (pdbg == pdbgmax) {
      ...
      dbgmod =
          (unsigned short *)REALLOC(dbgmod, pdbgmax * sizeof(unsigned short));
      if (lstflag) {
        lststring = (char **)REALLOC(lststring, pdbgmax * sizeof(char *));
        lstflag = (unsigned char *)REALLOC(lstflag, pdbgmax * sizeof(char));
        lstend =
            (unsigned int *)REALLOC(lstend, pdbgmax * sizeof(unsigned int));
        memset((char *)lstflag + (pdbgmax - MAXDBGS), -1,
               MAXDBGS * sizeof(char));
        memset((unsigned int *)lstend + (pdbgmax - MAXDBGS), 0,
               MAXDBGS * sizeof(unsigned int));
      }
    }
    ...
    if (dbg & 2) {
      if (SkipLineInfo)
        ...
        for (; sizestring < (MAXLSTSTR - 1); sizestring++) {
          c = *ofs;
          ofs = ofs + 1;
          if (c == 13 || ofs == endinput)
            break;
          buf[sizestring] = c;
        }
        buf[sizestring] = 0;
        strbtrim(buf);
        if ((sizestring = strlen(buf)) == 0)
          lststring[pdbg] = NULL;
        else {
  ...
  if (dbg & 1) {
    ...
    for (i = 0; i < totalmodule; i++) {
      if ((startfileinfo + i)->numdline == 0) {
        ...
        if (totalmodule != i) {
          ...
          for (j = 0; j < pdbg; j++) {
            if (dbgmod[j] == (unsigned short)totalmodule)
              dbgmod[j] = (unsigned short)i;
          }
          i--;
        }
      }
    ...
    for (j = 1; j < pdbg; j++) {
      if ((unsigned short)omodule != dbgmod[j]) {
        (corinfo + numcorrel)->count =
            (unsigned short)(j - (corinfo + numcorrel)->startline);
        (corinfo + numcorrel)->end = dbgloc[j] - 1;
        numcorrel++;
        corinfo = (struct _COR_INFO_ *)REALLOC(corinfo, sizeof(_COR_INFO_) *
                                                            (numcorrel + 1));
    ...
    (corinfo + numcorrel)->count =
        (unsigned short)(pdbg - (corinfo + numcorrel)->startline);
    (corinfo + numcorrel)->end = dbgloc[j - 1] + 1;
    numcorrel++;
    hout = CreateOutPut("tds", "wb");
    if (am32)
      retcode = CreateW32Debug();
    else
      retcode = CreateDosDebug();
    if (retcode == 0 && fwrite(output, outptr, 1, hout) != 1)
      retcode = -1;
    if (retcode != 0)
      ErrWrite();
    fclose(hout);
...
void AddNameToPul(char *name) {
  ...
  if ((lastofspul + i + 2) >= sizebuf) {
    sizebuf += STRLEN;
    if (sizebuf == STRLEN)
      bufname = (unsigned char *)MALLOC(sizebuf);
    else
      bufname = (unsigned char *)REALLOC(bufname, sizebuf);
  }
...
int CreateW32Debug() {
  ...
  for (; numsymbols < (short)totalmodule; numsymbols++)
    AddNameToPul((startfileinfo + numsymbols)->filename);
  ...
  for (i = 0, jj = 0; i < (unsigned int)numcorrel; i++) {
    if (i != 0)
      jj = jj + ((corinfo + i - 1)->count + 1) * 6 + 22;
    outdword(ofs + jj);
  }
  ...
  for (i = 0; i < (unsigned int)numcorrel; i++)
    outword(segcode); // массив индексов сегментов
  for (i = 0; i < (unsigned int)numcorrel; i++) {
    outword(1);                        // Segm#
    ...
    for (j = 0; j < jj; j++)
      outdword(dbgloc[j + ofs]);
    outdword((corinfo + i)->end);
    for (j = 0; j < jj; j++)
      outword(dbgnum[j + ofs]);
    outword(0);
  }
  ...
  if (fwrite(output, sstNames + 4, 1, hout) != 1)
    return -1;
  if (fwrite(bufname, lastofspul, 1, hout) != 1)
    return -1;
  free(bufname);
...
int CreateDosDebug() {
  ...
  d16header.sizeblock =
      sizeof(MODULE) * totalmodule + sizeof(SFT) * totalmodule +
      sizeof(LT) * pdbg + sizeof(SEGMENT) * totalmodule +
      sizeof(CT) * numcorrel + 6 * totalmodule +
      /*NUMTYPES*12+*/ sizeof(_SMB_) * numsymbols /*+numsymbols*4*/;
  ...
  for (i = 0; i < totalmodule; i++) {
    ...
    for (ii = 0; ii < numcorrel; ii++) { //обход таблицы корреляции
      if ((corinfo + ii)->file == (unsigned short)i) { //корр для этого модуля
        if (count == 0) {                              //первый блок
          ...
          (segment + i)->correli =
              (unsigned short)(corrnum + 1); // correlation index
    ...
    (segment + i)->correlc = (unsigned short)
        count; //(unsigned short)totalmodule; //correlation count
    (module + i)->corcount = (unsigned short)count;
    if (modelmem == TINY && comfile == file_exe)
      (segment + i)->segm = 0xfff0;
  }
  d16header.pol_size = outptr;
  if (fwrite(&d16header, sizeof(D16START), 1, hout) != 1)
    return -1;
  if (fwrite(symbols, sizeof(_SMB_) * numsymbols, 1, hout) != 1)
    return -1;
  if (fwrite(module, sizeof(MODULE) * totalmodule, 1, hout) != 1)
    return -1;
  free(module);
  if (fwrite(sft, sizeof(SFT) * totalmodule, 1, hout) != 1)
    return -1;
  free(sft);
  // line table
  ...
  if (fwrite(lt, sizeof(LT) * pdbg, 1, hout) != 1)
    return -1;
  free(lt);
  if (fwrite(segment, sizeof(SEGMENT) * totalmodule, 1, hout) != 1)
    return -1;
  free(segment);
  if (fwrite(ct, sizeof(CT) * numcorrel, 1, hout) != 1)
    return -1;
  free(ct);
  //	if(fwrite(types,NUMTYPES*12,1,hout)!=1)return -1;
  memset(&string3, 0, 6 * totalmodule);
  if (fwrite(&string3, 6 * totalmodule, 1, hout) != 1)
    return -1;
  /*	if(numsymbols){
...
void GeneratLst() {
  ...
  if (lstend[pdbg - 1] == 0)
    lstend[pdbg - 1] = endinptr;
  startip = (comfile != file_w32 && comfile != file_bin ? 0 : ImageBase);
  fprintf(hout,
          "SPHINX/SHEKER C-- One Pass Disassembler. Version %d.%02d%s %s\n",
  ...
  for (j = 0; j < pdbg; j++) {
    ...
    if ((int)lstflag[j] != -1) {
      ...
      if (offs2 != outptr) {
        /*
                                        if(line!=0)printf("%s
           %u:",(startfileinfo+dbgmod[j])->filename,line);
                                        if(lststring[j]!=NULL)printf("
           %s\n",lststring[j]); else if(line!=0)printf("\n");
                */
        fprintf(hout, "\n");
        if (line != 0)
          fprintf(hout, "%s %u:", (startfileinfo + dbgmod[j])->filename, line);
        if (lststring[j] != NULL)
          fprintf(hout, " %s\n", lststring[j]);
        else if (line != 0)
          fprintf(hout, "\n");
        while (outptr < offs2) {
          if (flag & 0x1e)
            undata(instruction_offset, offs2 - dbgloc[j], (flag >> 1) & 15);
          else
            unassemble(instruction_offset);
        }
      }
