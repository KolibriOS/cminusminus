...
int FindProcLib(int type) {
  ...
  for (;;) {
    ...
    if (strcmp((char *)itok.name, hlib.name) == 0) {
      int size;
      if ((outptr + hlib.size) >= outptrsize)
        CheckCodeSize();
      ...
      if (splitdata == 0)
        outptrdata = outptr;
      if (cpu < hlib.info[index].cpu)
        cpu = hlib.info[index].cpu;
      if (hlib.dosr[1] != 0) {
        if (hlib.dosr[1] > dos1) {
          dos1 = hlib.dosr[1];
          dos2 = hlib.dosr[0];
        } else if (hlib.dosr[1] == dos1 && hlib.dosr[0] > dos2)
          dos2 = hlib.dosr[0];
      }
      clearregstat();
#ifdef OPTVARCONST
...
void addtodefine(char *keystring) //добавить строку в дерево define
{
  ...
  if (tok == tk_string) {
    ...
    if (itok.rm == 1)
      newptr->sbuf = BackString((char *)string3);
  } else {
    if (string[0] != 0)
...
unsigned char get_directive_value() // return the 0 or 1 value for directive
{
  ...
  if (tok == tk_number) {
    if (doconstlongmath())
      return (1); //если значение не нулевое вернуть 1
    return (0);
  }
  numexpected();
  nexttok();
...
void InitDefineConst() {
  ...
  sprintf((char *)string, "%02d:%02d:%02d", timeptr.tm_hour, timeptr.tm_min,
          timeptr.tm_sec);
  itok.number = 8;
...
char *AddTextToBuf(char *buf, int *size, int start, int add) {
  ...
  *size += addsize;
  if (!buf) {
    ...
    buf = (char *)REALLOC(buf, *size + 1 + add);
...
void GetMacro(char *name) {
  ...
  while (tok != tk_closebracket && tok != tk_eof) {
    if (tok != tk_id)
      varexpected(numpar + 1);
    nsize = strlen(itok.name) + 1;
    if (size == 0)
      paramstr = (char *)MALLOC(nsize);
    else
      paramstr = (char *)REALLOC(paramstr, size + nsize);
...
void InitIdxRegs() {
  ...
  for (i = 0; tok == tk_reg32 || tok == tk_reg || tok == tk_beg; i++) {
    if (i < 4) {
      ...
      if (lreg[itok.number])
        preerror("expected unique register for \"pragma indexregs\"");
      lreg[itok.number] = 1;
    }
    nexttok();
    if (tok == tk_camma)
...
void directive() {
  ...
  case d_define:
    ...
    if (tok == tk_id || tok == tk_ID) {
      if (cha2 == '(') {
        ...
        if (tok != tk_endline && tok != tk_minus && tok != tk_number &&
            tok2 != tk_endline && tok2 != tk_semicolon) {
          inptr = longhold;
          cha = longhold2;
          ...
          case tk_minus:
            if (tok2 == tk_number) {
              if (i == tk_dword &&
                  (itok2.rm == tk_float || itok2.rm == tk_double ||
                   itok2.rm == tk_qword))
                i = itok2.rm;
            case tk_number:
              if (i == tk_dword &&
  ...
  case d_stack:
    ...
    if (tok == tk_number) {
      longhold = doconstlongmath();
      if (am32 == FALSE && (longhold > 0xFEFF || longhold < 0))
  ...
  case d_start:
    ...
    if (comfile == file_com) {
      if (tok == tk_number) {
        ...
        if (startStartup != 0x100)
          startStartup = startptr;
      } else {
        numexpected();
        nexttok();
      }
  ...
  case d_com: //список команд для SYS
    ...
    if (sysnumcom > MAXSYSCOM)
      preerror("to many commands");
    else if (sysnumcom != 0 && sysnumcom != MAXSYSCOM)
      listcom = (LISTCOM *)REALLOC(listcom, sysnumcom * sizeof(LISTCOM));
    break;
  ...
  case d_am32: // 32 битная адресация
    ...
    if (tok != tk_number)
      ...
      am32 = (unsigned char)(itok.number == 0 ? FALSE : TRUE);
    break;
  case d_undef: // undef
    oscanlexmode = scanlexmode;
    scanlexmode = DEFLEX2;
  ...
  case d_else:
    if (endifcount == -1)
      preerror("#else without preceeding #if");
    else if ((useelse[endifcount] & 1) == 0)
      preerror(toelse);
    else
      useelse[endifcount] &= 0xFE;
    if (gotoendif == 1)
      gotoendif = 0;
    else
      gotoendif = 1;
  ...
  case d_elif:
    if (endifcount == -1)
      preerror("#elif without preceeding #if");
    doifdef(d_elif);
    next = 0;
    gotoendif = 1;
    break;
  ...
  case d_pragma:
    ...
    if (strlen((char *)string))
      ptr = BackString((char *)string);
    else
      ptr = NULL;
    for (i = 0; i < p_end; i++)
      if (strcmp(itok.name, pragmalist[i]) == 0)
        break;
    if (i != p_li && ptr == NULL) {
    ...
    case p_st:
      ...
      ptr[i] = 0;
      if (numdomain == 0)
        domain = (char *)MALLOC(IDLENGTH);
      else
        domain = (char *)REALLOC(domain, IDLENGTH * (numdomain + 1));
    ...
    case p_re:
      ...
      //					retoldscanmode(STDLEX);
        ...
        preerror("for '#pragma resource' used only 'start' or 'end'");
      break;
    case p_db:
#ifdef DEBUGMODE
      if (strcmp(ptr, "start") == 0) {
        ...
        case tk_id:
          if (strcmp(itok.name, "push") == 0) {
            ...
            case 1:
              ...
              break;
            case 2:
              while (stackalign != NULL &&
                     strcmp(itok.name, stackalign->id) != 0) {
                newpar = stackalign->prev;
                free(stackalign);
                stackalign = newpar;
              }
  ...
  case d_inline:
    nexttok();
    if (tok == tk_number) {
      if (itok.number >= 0 && itok.number <= 2)
        useinline = itok.number;
      else
        preerror(ido);
    } else if (strcmp(itok.name, "AUTO") == 0)
      useinline = 2;
    else
      preerror(ido);
    break;
...
int ifdefconst() {
  if (optimizespeed == TRUE) {
    if (strcmp((char *)string, "speed") == 0)
      return TRUE;
  } else if (strcmp((char *)string, "codesize") == 0)
    return TRUE;
  ...
  if (comfile == file_w32 && strcmp((char *)string, "_WIN32") == 0)
    return TRUE;
  if (*(short *)&string[0] == 0x5F5F) {
    if (comfile == file_w32) {
      if (strcmp((char *)string + 2, "TLS__") == 0)
        return TRUE;
      if (dllflag && strcmp((char *)string + 2, "DLL__") == 0)
        return TRUE;
      else {
        if (wconsole && strcmp((char *)string + 2, "CONSOLE__") == 0)
          return TRUE;
        else if (strcmp((char *)string + 2, "WIN32__") == 0)
          return TRUE;
      }
    }
    if (am32) {
      if (strcmp((char *)string + 2, "FLAT__") == 0)
        return TRUE;
    } else {
      if (strcmp((char *)string + 2, "MSDOS__") == 0)
        return TRUE;
      if (modelmem == TINY && strcmp((char *)string + 2, "TINY__") == 0)
        return TRUE;
      if (modelmem == SMALL && strcmp((char *)string + 2, "SMALL__") == 0)
        return TRUE;
    }
    if (comfile == file_d32 && strcmp((char *)string + 2, "DOS32__") == 0)
      return TRUE;
    if (comfile == file_com && strcmp((char *)string + 2, "COM__") == 0)
      return TRUE;
    if (comfile == file_sys && strcmp((char *)string + 2, "SYS__") == 0)
      return TRUE;
    if (comfile == file_rom && strcmp((char *)string + 2, "ROM__") == 0)
      return TRUE;
    if (comfile == file_bin && strcmp((char *)string + 2, "BIN32__") == 0)
      return TRUE;
    if (comfile == file_meos && strcmp((char *)string + 2, "MEOS__") == 0)
      return TRUE;
    if ((sobj || fobj) && strcmp((char *)string + 2, "OBJ__") == 0)
      return TRUE;
    if (comfile == file_exe) {
      if (modelmem == TINY) {
        if (strcmp((char *)string + 2, "TEXE__") == 0)
          return TRUE;
      } else if (strcmp((char *)string + 2, "EXE__") == 0)
        return TRUE;
    }
  }
  return 2;
...
void CheckNumIF() {
  if (endifcount >= NUMIFDEF)
    preerror("#ifdef/#ifndef - too large enclosure");
}
...
void IncludeFile(char *fileincl, int tfind) {
  ...
  if (endifcount != oendifcount) {
    sprintf((char *)string2, "num if prior %d after %d", oendifcount,
            endifcount);
...
int startlabl(char *namelab) {
  // ITOK btok;
  // int bb;
  //	if(searchtree(&btok,&bb,(unsigned char
  //*)namelab)==FALSE)thisundefined(namelab);
  searchvar(namelab);
  return itok.number;
}
...
void searchvar(char *name, int err) {
  ...
  if (searchtree(&itok, &tok, string) == FALSE && err)
    thisundefined(name);
}
                  ...
                  //main...
{
  ...
  if (startuptomain == TRUE && comfile == file_com) {
    if (notdoneprestuff == TRUE) {
      ...
      if (jumptomain != CALL_NONE) {
        ...
        tobedefined(jumptomain == CALL_NEAR ? JMP_NEAR : CALL_SHORT,
                    tk_void); /* put main on the to be defined stack */
  ...
  if (comfile == file_meos)
    outptrdata = outptr = startptr + sizeof(MEOSheader);
  if (sobj) {
  ...
  if (comfile == file_bin)
    ...
    ImageBase = Align(ImageBase, 0x10000);
  if (jumptomain == CALL_NONE) {
    if (comfile == file_w32 || comfile == file_exe || comfile == file_meos)
      goto endp;
    if (comfile == file_com && fargc == 0 && parsecommandline == 0 &&
        atex == 0 && use_env == 0 && clearpost == 0 && resizemem == 0 &&
        killctrlc == 0)
      goto endp;
  }
  if (comfile == file_sys) {
    ...
    if (sysnumcom == 0)
      preerror("list command expected");
    else
      addconsttotree("__SYSNUMCOM", sysnumcom);
  ...
  if (comfile == file_w32 && wbss)
    clearpost = FALSE;
  if (resizemem)
    addconsttotree("__resizemem", TRUE);
  if (killctrlc)
    addconsttotree("__ctrl_c", TRUE);
  if (parsecommandline)
    addconsttotree("__parsecommandline", TRUE);
  if (atex)
    addconsttotree("__atexit", TRUE);
  if (fargc)
    addconsttotree("__argc", TRUE);
  if (use_env)
    addconsttotree("__environ", TRUE);
  if (clearpost)
    addconsttotree("__clearpost", TRUE);
  if (comfile == file_d32) {
    if (useDOS4GW)
      addconsttotree("__useDOS4GW", TRUE);
    if (dpmistub && (!usestub)) {
  ...
  if (jumptomain == CALL_SHORT)
    addconsttotree("__shortjmp", TRUE);
  else if (jumptomain == CALL_NONE)
    addconsttotree("__nonejmptomain", TRUE);
...
endp:
  ...
  free(bstring);
  if (strcmp(itok.name, mesmain) == 0)
    searchtree(&itok, &tok, (unsigned char *)&string);
...
void KillVarOfTree(idrec **treestart) {
  ...
  while (ptr != NULL && (cmpresult = strcmp(ptr->recid, itok.name)) != 0) {
    prev = ptr; //родитель
    ocmpresult =
        cmpresult; //результат пред сравнения - опр в левой или правой ветви
    if (cmpresult < 0)
      ptr = ptr->left;
    else
      ptr = ptr->right;
  ...
  if (ptr != NULL) { //найден объект удаления
    ...
    if (leftptr == NULL && rightptr == NULL) { //если нет дитей
      if (ocmpresult < 0)
        prev->left = NULL; //то родитель остался сиротой
      else if (ocmpresult > 0)
        prev->right = NULL;
      else
        *treestart = NULL; //удален корень без ветвей
    } else if (leftptr == NULL) { //одно дите справа
      if (ocmpresult < 0)
        prev->left = rightptr; //передать внуков родителю
      else if (ocmpresult > 0)
        prev->right = rightptr;
      else
        *treestart = rightptr; //удален корень с одной правой веткой
      ...
      while (((cmpresult = strcmp(ptrf->recid, ostptr->recid)) < 0 &&
              ptrf->left != NULL) ||
      ...
      if (cmpresult < 0)
        ptrf->left = ostptr; //строка меньше
      else
        ptrf->right = ostptr;
    }
    if (ptr->newid)
