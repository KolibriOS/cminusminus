  ...
  for (i = 0; i < 26; i++) {
    ...
    if (y != -1) {
      ...
      for (;; ii++) {
        ...
        fprintf(out, "\t0x%02X,0x%02X,", (unsigned char *)*ii,
                (unsigned char *)*(ii + 1));
        ii += 2;
        jj = ii;
        for (; *ii != 0; ii++)
          fprintf(out, "0x%02X,", (unsigned char *)*ii);
        fprintf(out, "0,\t// %c%s\n", i + 'A', jj);
      }
    }
  }
...
void InitList(char **keylist, int type)
/*-----------------22.11.99 19:35-------------------
 type: 0 - 26 больших букв с возможностью пропуска - строка "-" 2-байтовый
 идентификатор 1 - 26 маленьких букв и 'D' 2 - 26 маленьких и больших букв и
 символ '_' 2-юайтовый идентификатор
        --------------------------------------------------*/
{
  unsigned char *i;
  short j;
