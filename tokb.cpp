...
void Float2reg32(int reg, int addop = 0, int reg1 = idxregs[0],
                 int reg2 = idxregs[1]);
...
void cwpointr(ITOK *wtok, char *&wbuf, SINFO *wstr, int *otok, int npointr,
              int ureg);
int CheckAddOnly();
...
unsigned long leanum[24] = {3,   5,    9,    15,  25,   27,   45,   81,
                            ...
                            675, 1215, 2187, 625, 1125, 2025, 3645, 6561};

unsigned int numleamul[24][4] = {
    {3, 0, 0, 0}, {5, 0, 0, 0}, {9, 0, 0, 0}, {3, 5, 0, 0}, {5, 5, 0, 0},
    {3, 9, 0, 0}, {5, 9, 0, 0}, {9, 9, 0, 0}, {5, 5, 3, 0}, {5, 5, 5, 0},
    {9, 5, 3, 0}, {9, 5, 5, 0}, {9, 9, 3, 0}, {9, 9, 5, 0}, {9, 9, 9, 0},
    {5, 5, 5, 3}, {5, 5, 9, 3}, {9, 9, 5, 3}, {9, 9, 9, 3}, {5, 5, 5, 5},
    {9, 5, 5, 5}, {9, 9, 5, 5}, {9, 9, 9, 5}, {9, 9, 9, 9}};
  ...
  if (razr == r8) {
    ...
    if (num == 1) {
      ...
      if (chip == 0) {
        ...
        if (chip > 2 && optimizespeed && leamul32(li[num], reg, razr))
          return;
        op66(razr);
        op(0xC1);
...
int leamul32(unsigned long num, int reg, int razr) {
  ...
  op67(r32); // AX * n = LEA AX,[EAX*n+?EAX]
  op(0x8d);
  if (vop != 0x85 && vop != 0xc5 && reg == BP)
    i |= rm_mod01;
  op(i);
  op(vop + 8 * reg);
  if (vop == 0x85 || vop == 0xc5)
    outdword(0L);
  else if (reg == BP)
    op(0);
  return TRUE;
...
int speedmul32(unsigned long num, int reg, int razr) {
  ...
  if (reg == EAX)
    return speedmul(num, r32);
  return FALSE;
}

int speedmul(unsigned long num,
             int razr) //поиск возможности замены умножения на
//сдвиги и сложения
{
  int first, second;
  for (unsigned int j = 1; j < NUMNUM; j++) {
  ...
  for (;;) {
    ...
    if ((tok == tk_plus || tok == tk_minus) &&
        (tok2 == tk_minus ||
         ...
         (tok2 == tk_postnumber && posttok->post == 0) ||
         (tok == tk_plus && tok2 == tk_rmnumber &&
          (itok2.flag & f_useidx) == 0))) {
      goto con1;
    }
    if ((tok != tk_plus && (!(tok2 == tk_number && itok2.rm != tk_float &&
                              itok2.rm != tk_double))) ||
        (tok2 != tk_reg32 && tok2 != tk_number) || tok2 == tk_rmnumber) {
      break;
    }
    if (tok == tk_plus && tok2 == tk_reg32 &&
        (itok2.number == reg || (*idx != -1 && *base != -1))) {
      break;
    }
    if (tok == tk_mult &&
        (tok2 != tk_number || *zoom != -1 || *val != 0 || *idx != -1))
      break;
  con1:
    if (tok == tk_minus) {
      if (tok2 == tk_postnumber ||
          (tok2 == tk_number && (itok2.flag & f_reloc)))
        break;
      nexttok();
      if (tok == tk_minus) {
        if (tok2 != tk_number)
      ...
      *zoom = 0;
      ...
      break;
    nexttok();
    if (*base != -1 && *idx != -1 && tok2 != tk_number)
      break;
  }
  if (*zoom == -1 && *base == -1) {
    *base = *idx;
...
int OutLea(int reg, int idx, int base, int zoom, unsigned long val,
  ...
  if (zoom == -1)
    zoom = 0;
  if (val != 0) {
    if (short_ok(val, TRUE) != 0)
      mod = rm_mod01;
    else
      mod = rm_mod10;
  }
  if ((rflag & f_reloc) != 0 || posttok->post)
    mod = rm_mod10;
  if (idx == -1) {
    idx = base;
    base = -1;
  ...
  if (mod == rm_mod01)
    ...
    if (posttok->post)
      setwordpost(posttok);
    else if ((rflag & f_reloc) != 0)
      AddReloc();
    outdword(val);
  } else if (mod == rm_mod00 && base == 5)
    outdword(val);
  return TRUE;
}
...
int Reg32ToLea2(int reg) //оптимизация сложения 32-битных регистров в LEA
{
  ...
  ITOK oitok;
  ITOK posttok;
  if (tok != tk_minus && tok != tk_plus && tok != tk_mult)
    return FALSE;
  if (tok == tk_minus &&
      (!((tok2 == tk_number && itok2.rm != tk_float && itok2.rm != tk_double) ||
         tok2 == tk_postnumber)))
  ...
  if (tok == tk_mult &&
      (!(tok2 == tk_number && itok2.rm != tk_float && itok2.rm != tk_double &&
         (itok2.flag & f_reloc) == 0)))
    return FALSE;
  if (tok == tk_plus && tok2 != tk_reg32 && tok2 != tk_number &&
      tok2 != tk_postnumber &&
      (!(tok2 == tk_rmnumber && (itok2.flag & f_useidx) == 0)))
    return FALSE;
  if (cur_mod)
    return FALSE;
  posttok.post = 0;
  ...
  if (OutLea(reg, idx, base, zoom, val, rflag, &posttok) == FALSE)
    goto retfalse;
  ClearReg(reg);
  //	printf("%s (%u) Combination
...
int Reg32ToLea(int reg) //оптимизация сложения 32-битных регистров в LEA
{
  ...
  ITOK oitok;
  ITOK posttok;
  if (tok != tk_reg32 && tok != tk_number && tok != tk_postnumber &&
      (!(tok == tk_rmnumber && (itok.flag & f_useidx) == 0)))
    return FALSE;
  if (cur_mod)
    return FALSE;
  posttok.post = 0;
  ...
  if (tok == tk_number) {
    if (itok.rm == tk_float || itok.rm == tk_double || itok.rm == tk_qword)
      return FALSE;
    val = doconstdwordmath();
    rflag = postnumflag;
    if ((rflag & f_reloc) && tok == tk_mult)
      goto retfalse;
  } else if (tok == tk_postnumber) {
    posttok = itok;
    tok = tk_number;
  ...
  if (tok == tk_mult) {
    ...
    case 3:
      //				if(!(am32==FALSE&&tok!=tk_plus&&(!(tok2==tk_minus||tok2==tk_number)))){
      if (am32 || ((tok == tk_plus || tok == tk_minus) &&
                   (tok2 == tk_number || tok2 == tk_minus))) {
        zoom++;
        base = idx;
        break;
      }
  ...
  case 12: // val=0   zoom=-1         Rb+Ri
    ...
    break;
  case 10: // val=0   base=-1         Ri*Z
    if (optimizespeed == FALSE || idx == reg)
      goto retfalse;
    break;
  case 2: // base=-1                 Ri*Z+N
    if (optimizespeed)
...
void CheckRegForLea16(int reg, int *idx, int *base, unsigned long *val,
                      unsigned int *rflag, ITOK *posttok) {
  int endloop = FALSE;
  for (;;) {
    if (tok != tk_plus && (!(tok == tk_minus &&
                             ((tok2 == tk_number && itok2.rm != tk_float &&
                               itok2.rm != tk_qword && itok2.rm != tk_double) ||
                              tok2 == tk_postnumber))))
      break;
    if (tok2 == tk_postnumber && (posttok->post || tok == tk_minus))
      break;
    if (tok2 == tk_reg) {
      if (itok2.number == reg)
        endloop = TRUE;
      ...
      if (endloop)
        break;
      nexttok();
    } else if (tok2 == tk_number && itok2.rm != tk_float &&
               itok2.rm != tk_qword && itok2.rm != tk_double) {
      if (tok == tk_plus)
        *val += itok2.number;
      else
...
void OutLea16(int reg, int idx, int base, unsigned int val, int rflag,
              ITOK *posttok) {
  int mod = rm_mod00;
  int rm;
  if (val != 0) {
    if (short_ok(val, FALSE) != 0)
      mod = rm_mod01;
    else
      mod = rm_mod10;
  }
...
int Reg16ToLea(int reg) //оптимизация сложения 16-битных регистров в LEA
{
  ...
  if (!((tok == tk_reg ||
         (tok == tk_number && itok.rm != tk_float && itok.rm != tk_double) &&
             tok == tk_postnumber) &&
        tok2 == tk_plus))
    return FALSE;
  idx = base = -1;
...
int OptimNum() //оптимизация цифровых операндов
{
  ...
  int plusreloc = 0;
  if (optnumber == FALSE || cur_mod)
    return FALSE; //оптимизация отключена
  if (tok == tk_minus && (itok2.flag & f_reloc))
    return FALSE;
  deistv = 0;
  negflag = 0;
  starttok = otok = tok;
  ...
  case tk_plus:
    for (;;) { //оптимизация сложений-вычитаний
      ...
      if ((tok != tk_plus && tok != tk_minus) || tok2 != tk_number ||
          (tok == tk_minus && (itok2.flag & f_reloc) &&
           plusreloc == 0)) { //не цыфра или другое действие
        tok = otok;
        cha2 = ocha2;
        inptr2 = oinptr;
        endoffile = 0;
        if (deistv == 0)
          break; //ничего не было оптимизировано
        warningoptnum();
        tok = tk_plus;
        itok.lnumber = val;
        itok.flag |= flag;
  ...
  case tk_mult:
    for (;;) {
      nexttok();
      if ((tok != tk_div && tok != tk_mult && tok != tk_divminus &&
           tok != tk_multminus) ||
          tok2 != tk_number ||
          (otok != tok &&
           ((val > itok2.lnumber && (val % itok2.lnumber) != 0) ||
...
void RegMulNum(int reg, unsigned long num, int razr, int sign, int *expand,
               int flag) {
  int vop, i = 0;
  if ((flag & f_reloc) != 0)
    goto num_imul;
  if (razr == r16)
    num &= 0xffff;
  if (chip > 2 && (*expand == FALSE) && optimizespeed &&
      leamul32(num, reg, razr)) {
    setzeroflag = FALSE;
    return;
  }
  switch (num) {
  ...
  default:
    ...
    if (chip < 7 && *expand == FALSE && optimizespeed &&
        speedmul32(num, reg, razr))
      break;
  num_imul:
    if ((razr == r16 && chip < 2) || (*expand == TRUE)) {
      if (reg == AX) {
        ...
        op66(razr);
        if (sign)
          outword(0xE9F7); /* IMUL CX */
        else
          outword(0xE1F7); /* MUL CX */
      ...
      if ((flag & f_reloc) == 0 && short_ok(num, razr / 2 - 1))
        i = 2; //короткая форма
      op66(razr);
      op(0x69 + i); // imul
      op(0xc0 + reg * 9);
...
void MultiAssignFloat(int type, int npointr = 0) {
  ...
  if (numpointr > itok.npointr)
    unuseableinput();
  if (tok2 == tk_assign)
    MultiAssignFloat(type, numpointr);
  else {
    if (tok == tk_pointer)
      cpointr(BX, numpointr);
  ...
  if (otok == tk_pointer) {
    if (wtok.type == tk_proc) {
      ...
      if (npointr <= wtok.npointr)
        getpointeradr(&wtok, wbuf, &wstr, npointr - 1, razr, BX);
      else
        unuseableinput();
      wtok = itok;
    }
  }
  if ((wtok.rm == rm_d16 && wtok.sib == CODE16) ||
    ...
    if (am32 == FALSE)
      ...
      outdword(wtok.number);
  } else {
    CheckAllMassiv(wbuf, r32, &wstr, &wtok);
    op66(r32);
    outseg(&wtok, 2);
    op(0x89);
    op(wtok.rm); /* MOV [rmword],reg */
    outaddress(&wtok);
  }
}
...
int MultiAssign(int razr, int usereg, int npointr) {
  ...
  if (otok == tk_pointer)
    cwpointr(&wtok, wbuf, &wstr, &otok, npointr, ureg);
  switch (otok) {
  case tk_intvar:
  case tk_wordvar:
  case tk_longvar:
  ...
  case tk_floatvar:
    ...
    if (hnumber == 0 &&
      ...
      outseg(&wtok, 2);
      op(0x89);
      op(wtok.rm + hnumber * 8); /* MOV [rmword],reg */
      outaddress(&wtok);
    }
    break;
  case tk_charvar:
  ...
  case tk_reg32:
    if (wtok.number != hnumber) {
      if (RegToReg(hnumber, wtok.number, nrazr) == NOINREG) {
        ...
        waralreadinitreg(regs[nrazr / 4][wtok.number],
                         regs[nrazr / 4][hnumber]);
    }
    break;
  case tk_beg:
    if (razr > r8 && wtok.number > 3 && (wtok.number % 4) == hnumber)
      preerror("register AH,BH,CH,DH should be first");
    if (wtok.number != hnumber) {
      if (RegToReg(hnumber, wtok.number, r8) == NOINREG) {
        op(0x88);
        op(0xC0 + wtok.number + hnumber * 8); // mov beg,AL
      } else
        waralreadinitreg(begs[wtok.number], begs[hnumber]);
    }
    break;
  case tk_seg:
    op(0x8E); /* MOV SEG,AX */
...
int do_d_wordvar(
    int sign, int razr,
    int terminater) // signed or unsigned 16 or 32 bit memory variable
{
  ...
  posiblret = rettype = (sign == 0 ? (razr == r16 ? tk_word : tk_dword)
                                   : (razr == r16 ? tk_int : tk_long));
  wstr = strinf;
  strinf.bufstr = NULL;
  wtok = itok;
  wbuf = bufrm;
  bufrm = NULL;
  otok = tok;
  while (RmEqualReg(hnumber, itok.rm, itok.sib))
    hnumber++;
  nexttok();
#ifdef OPTVARCONST
  operand = tok;
  ...
  case tk_assign: //=
    ...
    if (tok == tk_pointer)
      cpointr(am32 == TRUE ? EAX : BX, numpointr);
    CheckMinusNum();
    //			printf("tok=%d tok2=%d\n",tok,tok2);
...
// labl1;
      ...
      loadconst:
        if ((itok.flag & f_reloc) == 0) {
          ...
          if ((razr == r32 && itok.number == 0xFFFFFFFF) ||
              (razr == r16 && itok.number == 0xFFFF)) {
            CheckAllMassiv(wbuf, razr, &wstr, &wtok);
            op66(razr);
            outseg(&wtok, 2);
            op(0x83);
      ...
      numbertovar:
        ...
        else if ((itok.flag & f_reloc) != 0)
          AddReloc();
        if (razr == r16) {
          if (am32 != FALSE && tok != tk_number)
            dwordvalexpected();
      ...
      case tk_longvar:
      case tk_dwordvar:
        if ((rettype == tk_long || rettype == tk_dword) && hnumber &&
            regoverstack)
          goto pushvar;
        goto labl1;
      case tk_intvar:
      case tk_wordvar:
        if ((rettype == tk_int || rettype == tk_word) && hnumber &&
            regoverstack) {
        pushvar:
    ...
    getfromax:
      ...
      convert_returnvalue(posiblret, rettype);
      if (addESP != oaddESP && am32 && ESPloc &&
          (wtok.type == tp_paramvar || wtok.type == tp_localvar))
      ...
      if (wbuf == NULL && wstr.bufstr == NULL && hnumber == 0 &&
        ...
        outseg(&wtok, 2);
        op(0x89);
        op(wtok.rm + hnumber * 8); // MOV [rmword],AX
        outaddress(&wtok);
      }
      if (ofsstr)
  ...
  case tk_plusequals:
    ...
    axtovar:
      ...
      num:
        ...
        outseg(&wtok, 2);
        if (tok == tk_number && (itok.flag & f_reloc) == 0 &&
            itok.number == 1 && (vop == 0 || vop == 0x28)) {
          if (vop)
            vop = 8;
          op(0xFF);
          op(vop + wtok.rm);
          outaddress(&wtok);
        } else if (tok != tk_undefofs && tok != tk_postnumber &&
                   (itok.flag & f_reloc) == 0 &&
  ...
  case tk_swap:
    ...
    rbuf = bufrm;
    bufrm = NULL;
    if (am32 != FALSE && wbuf != NULL && wstr.bufstr != NULL)
      regdi = FALSE;
    switch (tok) {
    case tk_reg32:
      if (razr == r16)
    ...
    case tk_intvar:
    case tk_wordvar:
      if ((tok == tk_intvar || tok == tk_wordvar) && razr == r32)
        swaperror();
#ifdef OPTVARCONST
      ClearVarByNum(&itok);
#endif
      if (hnumber == 0)
        ...
        if (regoverstack &&
            (!((bufrm || strinf.bufstr) && (wbuf || wstr.bufstr)))) {
          CheckAllMassiv(bufrm, razr, &strinf);
          op66(razr);
          outseg(&itok, 2);
          op(0xFF); // PUSH [dword]
      ...
      CheckAllMassiv(rbuf, razr, &strinf, &itok,
                     regdi == FALSE ? idxregs[2] : idxregs[1], idxregs[3]);
      op66(razr);
      outseg(&itok, 2);
    ...
    case tk_floatvar:
      ...
      if (sign == 1) {
        ...
        op(wtok.rm);
        outaddress(&wtok);
        CheckAllMassiv(rbuf, 4, &strinf, &itok,
                       regdi == FALSE ? idxregs[2] : idxregs[1], idxregs[3]);
  ...
  case tk_llequals:
    ...
    if (itok2.type != tp_stopper) {
      ...
      outseg(&wtok, 2);
      op(0xD3);
      op(0x20 + vop + wtok.rm); // SHL [rmword],CL
      outaddress(&wtok);
      warningreg(begs[1]);
      next = 0;
      ...
      if ((unsigned int)itok.number == 1) {
        ...
        outseg(&wtok, 2);
        op(0xD1);
        op(0x20 + vop + wtok.rm); /* SHL [rmword],1 */
        outaddress(&wtok);
        ...
        if (chip < 2 && razr == r16) {
          ...
          outseg(&wtok, 2);
          op(0xC1);
          op(0x20 + vop + wtok.rm); /* SHL [rmword],imm8 */
          outaddress(&wtok);
          if (cpu < 2)
            cpu = 2;
        }
...
int dobytevar(int sign, int terminater) // byte, char
{
  ...
  switch (tok) {
  case tk_assign:
    if (!((tok2 == tk_reg || tok2 == tk_reg32 || tok2 == tk_beg) &&
          ScanTok3() == terminater)) {
      ofsstr = GetLecsem(terminater);
    }
    if (ofsstr) {
    ...
    if (itok2.type == tp_opperand) {
      if (rettype != tk_float &&
          tok == tk_number) { //проверка и суммирование чисел
        if (OnlyNumber(sign)) {
          next = 0;
          goto numbertovar;
        }
      ...
      numbertovar:
#ifdef OPTVARCONST
        if ((initconst = Const2Var(&btok, itok.lnumber & 0Xff, itok.rm)) ==
            FALSE) {
    ...
    getfromax:
      ...
      convert_returnvalue(posiblret, rettype);
      if (addESP != oaddESP && am32 && ESPloc &&
          (btok.type == tp_paramvar || btok.type == tp_localvar))
        btok.number += addESP - oaddESP;
      if (bbuf == NULL && bstr.bufstr == NULL && hnumber == 0 &&
          ((btok.rm == rm_d16 && btok.sib == CODE16) ||
           (btok.rm == rm_d32 && (btok.sib == CODE32 || btok.sib == 0)))) {
        outseg(&btok, 1);
        op(0xA2); // MOV [byte],AL
  ...
  case tk_swap:
    ...
    case tk_charvar:
      ...
      if (hnumber == 0)
        ...
        getinto_reg(otok, &btok, bbuf, &bstr, r8, hnumber);
      CheckAllMassiv(
          rbuf, 1, &strinf, &itok,
          (am32 != FALSE && bbuf != NULL && bstr.bufstr != NULL) ? BX : DI, DX);
      outseg(&itok, 2);
  ...
  case tk_llequals:
    ...
    if (itok2.type != tp_stopper) {
      ...
      outseg(&btok, 2);
      op(0xD2);
      op(0x20 + vop + btok.rm); /* SHL [byte],CL */
      outaddress(&btok);
      warningreg(begs[1]);
      ClearReg(CX);
      ClearReg(AX);
      ...
      if ((unsigned int)itok.number == 1) {
        ...
        outseg(&btok, 2);
        op(0xD0);
        op(0x20 + vop + btok.rm); /* SHL [byte],1 */
        outaddress(&btok);
      } else if ((unsigned int)itok.number != 0) {
        CheckAllMassiv(bbuf, 1, &bstr, &btok);
        if (chip < 2) {
          ...
          outseg(&btok, 2);
          op(0xC0);
          op(0x20 + vop + btok.rm); /* SHL [byte],imm8 */
          outaddress(&btok);
          if (cpu < 2)
            cpu = 2;
        }
...
void getinto_e_ax(int sign, int gtok, ITOK *gstok, char *&gbuf, SINFO *gstr,
                  int razr, int useAX) {
  unsigned int i = 0;
  int vop = 0;
  int reg1 = idxregs[0], reg2 = idxregs[1];
  ...
  case tk_postnumber:
    op66(razr);
    op(0xB8); /* MOV EAX,# */
    (gstok->flag & f_extern) == 0 ? setwordpost(gstok)
                                  : setwordext(&gstok->number);
  ...
  case tk_longvar:
    i += 4;
  longvar:
    if ((gstok->rm == rm_d16 && gstok->sib == CODE16) ||
        (gstok->rm == rm_d32 && (gstok->sib == CODE32 || gstok->sib == 0))) {
      op66(razr);
      outseg(gstok, 1);
      op(0xA1);
      if (gstok->post == UNDEF_OFSET)
        AddUndefOff(2, gstok->name);
      if (am32 == FALSE)
        outword((unsigned int)gstok->number);
      else
        outdword(gstok->number);
    } else {
      CheckAllMassiv(gbuf, i, gstr, gstok, reg1, reg2);
      op66(razr);
      outseg(gstok, 2);
  ...
  case tk_wordvar:
    ...
    if (razr == r16)
      goto longvar;
    i = 1;
    if (optimizespeed && vop == 0 && chip > 3 && chip < 7)
      goto optpent;
  movxx:
  ...
  optpent:
    ...
    if ((chip >= 3 && (!optimizespeed)) ||
        RmEqualReg(AX, gstok->rm, gstok->sib))
      goto movxx;
    ZeroReg(EAX, razr);
    ...
    if (optimizespeed && chip > 3 && chip < 7) {
      if (sign) {
        ...
        if (razr == r32 && (gstok->number == AL || gstok->number == AH)) {
          ...
          /*						op(0x88);
                                                          op(0xC1+gstok->number*8);
             //mov cl,beg xorEAXEAX(); outword(0xC888);	// mov al,cl
                                                          warningreg(begs[1]);
                                                          break;*/
        }
        if (gstok->number == AH)
          outdword(0xE430E088); // mov al,ah xor ah,ah
        else {
          ZeroReg(EAX, razr);
          op(0x88);
          op(0xC0 + gstok->number * 8); // mov al,beg
    ...
    if (razr == r32) {
      ...
      if (optimizespeed && (!sign) && chip > 3 && chip < 7) {
        if (gstok->number == AX) {
          ...
          /*						op66(r16);
                                                          outword(0xC189);
             //mov cx,AX xorEAXEAX(); op66(r16); outword(0xC889);	//mov
             aX,cX warningreg(regs[0][1]);*/
        } else {
          xorEAXEAX();
          op66(r16);
          op(0x89);
  ...
  case tk_string:
    ...
    if (razr == r16) {
      ...
      outdword(addpoststring(CS, gstok->number, gstok->flag));
    ClearReg(AX);
    break;
  default:
    valueexpected();
...
//	printf("tok=%d %s\n",tok,itok.name);
  ...
  case tk_number:
    ...
    if (tok == tk_mult && (optimizespeed || razr == r32) &&
        (!((tok2 == tk_reg32 || tok2 == tk_reg) && itok2.number == EAX)) &&
        expandvar() == FALSE) {
      getoperand(am32 == TRUE ? EAX : BX);
      next = 1;
      goto loopswitch;
  ...
  case tk_rmnumber:
    ...
    if (itok.rm || am32 == 0) {
      ...
      if ((itok.rm & 0x3F) == 0 && am32 && itok.post == 0 &&
          (short_ok(itok.number, TRUE) == FALSE ||
           ((itok.rm & rm_mod11) == rm_mod10))) { // add
        outseg(&itok, 1);
        op(5); // add
      } else { // lea
        if (itok.post == 0)
  ...
  case tk_declare:
    if (itok.flag & f_retproc)
      rettype = (itok.flag & f_retproc) / 256 + tk_overflowflag - 1;
    if ((!i) || macros(sign != 0 ? (razr == r16 ? tk_int : tk_long)
                                 : (razr == r16 ? tk_word : tk_dword)) == 0) {
      procdo(sign != 0 ? (razr == r16 ? tk_int : tk_long)
                       : (razr == r16 ? tk_word : tk_dword));
    }
    nexttok();
    if (*ofsstr) {
      free(*ofsstr);
...
#endif
  if (next)
    RegMulNum(AX, holdnumber, razr, sign, &expand, i);
...
// !!! new
    ...
    if (uselea && razr == r32) {
      if (Reg32ToLea2(EAX))
        continue; //оптимизация сложения 32-битных регистров в LEA
      if (itok.type == tp_stopper || tok == tk_eof || itok.type == tp_compare)
        break;
    }
    ...
    case tk_plus:
      ...
      case tk_number:
        if ((itok.flag & f_reloc) == 0) {
          if (optnumadd(itok.number, 0, razr, vop))
            break;
        }
      case tk_undefofs:
      case tk_postnumber:
        ...
        if (tok == tk_postnumber)
          ...
          AddReloc();
        razr == r16 ? outword((unsigned int)itok.number)
                    : outdword(itok.number);
        setzeroflag = TRUE;
        break;
      case tk_apioffset:
      ...
      case tk_intvar:
      case tk_wordvar:
        if (razr == r32 && (tok == tk_intvar || tok == tk_wordvar))
          goto defxor;
        i += 2;
      ...
      case tk_declare:
        ...
        oaddstack = addstack;
        addstack = FALSE;
        procdo(razr == r16 ? (sign == 0 ? tk_word : tk_int)
                           : (sign == 0 ? tk_dword : tk_long));
        addstack = oaddstack;
        addESP -= razr == r16 ? 2 : 4;
        op66(razr);
        op(0x58 + EDX);
      ...
      if (expand == TRUE) {
        ...
        op(0x83);
        if (oldtok == tk_plus)
          outword(0x00d2); // adc dx,0
        else if (oldtok == tk_minus)
          outword(0x00da); // sbb dx,0
        setzeroflag = TRUE;
      }
      break;
    case tk_modminus:
    ...
    case tk_mult:
      ...
      mulreg32:
        ...
        op(0xF7);
        if (sign)
          op(0xE8 + (unsigned int)itok.number);
        else
...
void getintoal(int gtok, ITOK *gstok, char *&gbuf,
               SINFO *gstr) // AH may also be changed
{
  unsigned int i = 0;
  switch (gtok) {
  case tk_bits:
...
int doalmath(int sign, char **ofsstr) {
  ...
  case tk_declare:
    if (itok.flag & f_retproc)
      rettype = (itok.flag & f_retproc) / 256 + tk_overflowflag - 1;
    if ((!i) || macros(sign != 0 ? tk_char : tk_byte) == 0)
  ...
  if (negflag) {
    if (optimizespeed && (chip == 5 || chip == 6))
      outdword(0xC0FEFF34); // xor AL,-1 AL++
    else
...
void doalmath2(int sign) {
  ...
  while (itok.type != tp_stopper && tok != tk_eof && itok.type != tp_compare) {
    ...
    case tk_plus:
      ...
      switch (tok) {
      case tk_number:
        if (itok.number == 0 && oldtok != tk_and)
          break;
        if (itok.number == 255 && oldtok == tk_and)
          break;
        op(0x04 + vop);
    ...
    case tk_div:
      ...
      if (tok == tk_number) {
        ...
        if (vop) { //%
          ...
          if (caselong(itok.number) != NUMNUM) {
            ...
            op(0xB1);
            op((unsigned int)itok.number); /* MOV CL,# */
            if (sign)
              outword(0xF9F6); /* IDIV CL */
            else
              outword(0xF1F6); /* DIV CL */
          ...
          default:
            ...
            if (vop != NUMNUM) {
              ...
              if (expand == FALSE) {
                if (optimizespeed && (itok.flag & f_reloc) == 0 &&
                    sign == 0) { // for signed needed new algoritm
      ...
      case tk_doublevar:
        ...
        defdiv:
          op(0xF6);
          if (sign)
            op(0xF8 + (unsigned int)itok.number);
          else
            op(0xF0 + (unsigned int)itok.number);
          setzeroflag = FALSE;
          break;
        case tk_reg32:
        case tk_reg:
      ...
      case tk_number:
        ...
        default:
          ...
          if (vop != NUMNUM) {
            if (chip < 1) {
              if (expand == TRUE) {
                if (optimizespeed == FALSE && sign == FALSE)
                  goto num_imul;
                if (sign)
                  cbw();
                else
...
int doreg_32(int reg, int razr, int terminater) {
  ...
  case tk_assign: //=
    ...
    if (!((tok2 == tk_reg || tok2 == tk_reg32 || tok2 == tk_beg) &&
      ...
      if (i != razr || RegToReg(reg, itok.number, i) == NOINREG)
        goto nn1;
    ...
    //			printf("tok=%d %s\n",tok,itok.name);
    if (tok == tk_pointer)
      cpointr(am32 == TRUE ? reg : BX, numpointr);
  ...
  nn1:
    ...
    if (reg == AX) {
      ...
      if (rrettype == tk_char || rrettype == tk_byte && reg <= BX) {
        ...
        if (rrettype == tk_int || rrettype == tk_word)
          next = r16;
        else
          next = r32;
    ...
    swapreg:
      if (reg != (int)itok.number) {
        if (RegSwapReg(reg, itok.number, razr) == NOINREG) {
          ...
          waralreadinitreg(regs[razr / 4][reg], regs[razr / 4][itok.number]);
      }
      break;
    default:
      swaperror();
  ...
  case tk_plusequals:
    ...
    if (CheckAddOnly()) {
      ...
      if (reg == EAX)
        ...
        doregmath_32(reg, razr, 0, &ofsstr);
      next = 0;
      break;
    }
    getoperand(reg == BX ? SI : BX);
    ...
    case tk_number:
      ...
      if (itok.type == tp_opperand) {
        if (reg == EAX) {
          sign = ECX;
          if ((tok2 == tk_reg || tok2 == tk_reg32) && itok2.number == ECX)
            sign = EDX;
        ...
        if (i == tk_postnumber || i == tk_undefofs) {
          ...
          op(0xB8 + reg); // MOV reg,#
          if (i == tk_postnumber)
            (postnumflag & f_extern) == 0 ? setwordpost(&itok)
                                          : setwordext((long *)&ii);
          else {
        ...
        if (sign == EAX)
          ...
          doregmath_32(sign, razr, 0, &ofsstr);
        itok.number = sign;
        goto addreg;
      }
      if ((postnumflag & f_reloc) == 0 && i != tk_undefofs &&
          i != tk_postnumber && optnumadd(ii, reg, razr, vop))
        break;
    addnum:
      op66(razr);
      if (reg == AX)
  ...
  case tk_llequals:
    ...
    if (tok == tk_number) {
      ...
      if (!(itok2.type == tp_stopper &&
            (tok == tk_beg || tok == reg || tok == tk_reg32) &&
            itok.number == CL)) {
        getintobeg(CL, &ofsstr);
        dobegmath(CL);
        warningreg(begs[1]);
    ...
    if (tok == tk_number) {
      ...
      if (itok.type == tp_opperand) {
        ...
        if (sign == EAX)
          ...
          doregmath_32(ECX, razr, 0, &ofsstr);
      ...
      if (itok2.type == tp_stopper)
        next = (unsigned char)MulReg(reg, razr);
      else {
        if (reg == AX) {
          getintoreg_32(ECX, razr, sign, &ofsstr);
          doregmath_32(ECX, razr, sign, &ofsstr);
...
int optnumadd(unsigned long num, int reg, int razr, int vop) {
  ...
  if (!optimizespeed && num == 2 &&
      ((razr == r16 && am32 == FALSE) || (razr == r32 && am32))) {
    if (vop == 0x28) { //-=
      op(0x48 + reg);
      op(0x48 + reg);
      setzeroflag = TRUE;
  ...
  if (!optimizespeed &&
          (razr == r16 && (unsigned short)num == 0xfffe && am32 == FALSE) ||
...
int dobeg(int beg, int terminater) {
  ...
  case tk_assign:
    if (!((tok2 == tk_reg || tok2 == tk_reg32 || tok2 == tk_beg) &&
      ...
      nexttok();
      if (RegToReg(beg, itok.number, r8) == NOINREG)
        goto nn1;
    ...
    nexttok();
    convert_type(&sign, &rrettype, &pointr,
                 am32 == TRUE ? (beg > 3 ? beg - 4 : beg) : BX);
    while (tok == tk_mult) {
      nexttok();
      numpointr++;
    }
    ...
    if (tok == tk_pointer)
      cpointr(am32 == TRUE ? (beg > 3 ? beg - 4 : beg) : BX, numpointr);
  nn1:
    if (beg == AL) {
      if (rrettype == tk_char || rrettype == tk_byte)
        rettype = doalmath(sign, &ofsstr);
    ...
    case tk_beg:
      if (beg != (int)itok.number) {
        if (RegSwapReg(beg, itok.number, r8) == NOINREG) {
          ...
          waralreadinitreg(begs[beg], begs[itok.number]);
      }
      break;
    default:
      swaperror();
...
void doseg(int seg) {
  ...
  char *ofsstr = NULL;
  if (seg == CS)
    preerror("CS not used for destention");
  if (seg == FS || seg == GS)
    if (cpu < 3)
      cpu = 3;
  if (seg == SS)
  ...
  if (tok == tk_assign) {
    ...
    case tk_seg:
      ...
      break;
    default:
      preerror("Only int, word variables valid for segment register ><");
      break;
    }
  } else
    segoperror();
...
void doregmath_32(int reg, int razr, int sign, char **ofsstr,
                  int fdiv) // math done is on all regs except AX
// all other registers preserved
{
  ...
  while (itok.type != tp_stopper && tok != tk_eof) {
    ...
    case tk_plus:
      ...
      case tk_number:
        if ((itok.flag & f_reloc) == 0 &&
            optnumadd(itok.number, reg, razr, vop))
          break;
      case tk_postnumber:
      case tk_undefofs:
        op66(razr);
    ...
    case tk_rr:
      getoperand(am32 == TRUE ? ECX : (reg == BX ? SI : BX));
      if (RshiftReg(razr, reg, sign) == FALSE)
        regmathoperror();
      else
        continue;
      break;
    ...
    case tk_ll:
      ...
      if (tok == tk_number) {
        ...
        lshiftmul(itok.number, razr, reg);
                 ...
                 (tok == tk_beg || tok == tk_reg || tok == tk_reg32) &&
                 itok.number == CL) {
        op66(razr);
        op(0xD3);
...
int getintoreg_32(int reg, int razr, int sign, char **ofsstr,
                  int useloop) // get into word reg (except AX) with enum
{
...
loopswitch1:
  ...
  case tk_number:
    if (useloop == FALSE)
      MovRegNum(razr, itok.flag & f_reloc, itok.number, reg);
    else {
      holdnumber = CalcNumber(sign);
      if (loop == 0)
        oflag = postnumflag;
  ...
  case tk_rmnumber:
    CheckAllMassiv(bufrm, itok.size, &strinf, &itok, reg1, reg2);
    if ((am32 && itok.rm == reg && reg != EBP && reg != ESP) ||
        (am32 == 0 && reg == itok.rm && (reg == SI || reg == DI || reg == BX)))
      break;
    op66(razr);
    op67(itok.sib == CODE16 ? r16 : r32);
    if (itok.post == 0)
  ...
  case tk_wordvar:
    ...
    if (tok == tk_wordvar && optimizespeed && chip > 3 && chip < 7 &&
        RmEqualReg(reg, itok.rm, itok.sib) == FALSE) {
      ZeroReg(reg, r32);
      op66(r16);
  ...
  case tk_charvar:
    if (chip > 2 || razr == r32) {
      ...
      if (reg <= EBX && tok == tk_bytevar && optimizespeed && chip > 3 &&
          chip < 7 && RmEqualReg(reg, itok.rm, itok.sib) == FALSE) {
        ZeroReg(reg, razr);
        outseg(&itok, 2);
        op(0x8A);
      } else {
  ...
  case tk_beg:
    if (chip > 2 || razr == r32) {
      if (optimizespeed && chip > 3 && chip < 7 && reg < 4 &&
          reg != (int)(itok.number % 4)) {
        ZeroReg(reg, razr);
        op(0x88);
  ...
  case tk_declare:
    if (itok.flag & f_retproc)
      rettype = (itok.flag & f_retproc) / 256 + tk_overflowflag - 1;
    if ((!i) || macros(razr == r16 ? tk_word : tk_dword) == 0)
      procdo(razr == r16 ? tk_word : tk_dword);
    itok.number = 0;
...
int getintobeg(int beg,
               char **ofsstr) // get into beg (CL,DL,BL not others) with enum
{
  int negflag = 0, i = 0;
  int rettype = tk_beg;
  if (tok == tk_minus) {
...
int dofloatvar(int addop, int retrez, int terminater) {
  ...
  case tk_assign: //=
    ...
    if (itok2.type == tp_opperand) { //составное
      ...
      numbertovar:
        ...
        for (i = 0; i < 2; i++) {
          ...
          if ((itok.flag & f_reloc) == 0 && itok.number == 0) {
            ...
            outaddress(&wtok);
            op(0);
          } else if (regoverstack && short_ok(itok.number, TRUE) &&
                     (itok.flag & f_reloc) == 0) {
            op(0x6A);
            op(itok.number); // push short number
            op66(r32);
      ...
      case tk_floatvar:
        ...
        w1str = strinf;
        strinf.bufstr = NULL;
        getinto_e_ax(0, tok == tk_floatvar ? tk_dwordvar : tk_qwordvar, &w1tok,
                     w1buf, &w1str, r32);
        if (tok == tk_doublevar) {
          if ((wtok.rm == rm_d16 && wtok.sib == CODE16) ||
              (wtok.rm == rm_d32 && wtok.sib == CODE32)) {
            op66(r32);
            outseg(&wtok, 1);
    ...
    if (itok2.type == tp_stopper) {
      if (tok == tk_number) {
        if ((itok.rm == tk_float && itok.fnumber == 1.0) ||
            ...
            (itok.rm != tk_float && itok.rm != tk_double &&
             itok.lnumber == 1)) {
          if (vop == 0 || vop == 0x28)
            goto incvar;
          break;
        ...
        if ((itok.rm == tk_float && itok.fnumber == 0.0) ||
            (itok.rm == tk_double && itok.dnumber == 0.0) ||
            itok.lnumber == 0) {
          switch (vop) {
          case 0x38:
            DevideZero();
    ...
    endequals1:
      ...
      case tk_number:
        ...
        if (vop == 0x38) { // div 22.12.05 22:10
          vop = 8;         // mult
          if (itok.rm == tk_float)
            itok.fnumber = 1 / itok.fnumber;
          else
            itok.dnumber = 1 / itok.dnumber;
        }
  ...
  case tk_swap:
    ...
    case tk_reg32: //добавить обработку интерпритации float, long
      ...
      if (am32) {
        if (optimizespeed)
          outword(0x241c); // fistp ssdword[bp-8]/ssdword[esp]
        else {
          outword(0x245C);
          op(4);
        }
    ...
    case tk_longvar:
      ...
      break;
    case tk_qwordvar:
      CheckAllMassiv(rbuf, 8, &strinf, &itok,
                     regdi == FALSE ? idxregs[2] : idxregs[1], idxregs[3]);
...
void fildq2_stack(int size) {
  op(0xdf);
  if (am32)
    outword(0x242c); // fildq ssdword[bp-8]/ssdword[esp]
  else {
...
void fildq_stack() { fildq2_stack(localsize + 8); }

void fistp2_stack(int size) {
  if (am32)
    outword(0x241c); // fistp ssdword[ebp+2]/ssdword[esp]
  else {
    if (short_ok(size, FALSE) == 0) {
      op(0x9E);
      outword(-size);
...
void fld_stack(int size) {
  if (am32)
    outword(0x2404); // fld ssdword[ebp+2]/ssdword[esp]
  else {
    if (short_ok(size, FALSE) == 0) {
      op(0x86);
      outword(-size);
...
void reginstack(int *numstak, int *nums) {
  ...
  if (tok == tk_beg) {
    if (optimizespeed && chip > 3 && chip < 7 && itok.number != AL &&
        itok.number != AH) {
      xorAXAX();
      op(0x88);
      op(0xC0 + itok.number * 8); // mov al,beg
    } else if (itok.number == AL)
      xorAHAH();
    else {
...
void intinstack(int addop) {
  ...
  if (tok == tk_intvar || tok == tk_qwordvar)
    op(0xDF);
  else if (tok == tk_floatvar || tok == tk_doublevar)
    op(0xd9 + addop);
  else
    op(0xDB);
...
int doeaxfloatmath(int itreturn, int reg, int addop) {
  ...
  if (itreturn == tk_stackstart) {
    if (!am32) {
      ...
      op(0xD9 + addop);
      if (numstak == 0)
        outword(0x241C); // fstp ssdword[esp]
      else {
...
void AddReloc(int segm) {
  if (FixUp != FALSE) {
    ...
    if (segm == DS) {
      (postbuf + posts)->type =
          (unsigned short)(am32 == FALSE ? FIX_VAR : FIX_VAR32);
      (postbuf + posts)->loc = outptrdata;
    } else {
      (postbuf + posts)->type =
          (unsigned short)(am32 == FALSE ? FIX_CODE : FIX_CODE32);
...
void fwait3() {
  if (chip < 4 && tok2 != tk_floatvar && tok2 != tk_doublevar &&
      tok2 != tk_float && tok2 != tk_double)
    op(0x9B);
}
...
void AddFloatConst(long long fnumber, int type) {
  ...
  for (i = 0; i < numfloatconst; i++) {
    if (type == (floatnum + i)->type) {
      ofs = (floatnum + i)->ofs;
      if (type == tk_float) {
        if (*(long *)&fnumber == (floatnum + i)->num[0])
          goto endp;
      } else if (*(double *)&fnumber == (floatnum + i)->dnum)
        goto endp;
    }
  }
  if (numfloatconst == 0) {
    ...
    floatnum = (LISTFLOAT *)REALLOC(
        floatnum, (maxnumfloatconst + STEPFLOATCONST) * sizeof(LISTFLOAT));
    maxnumfloatconst += STEPFLOATCONST;
  }
  numfloatconst++;
  ...
  if (type == tk_float)
    (floatnum + i)->num[0] = *(unsigned long *)&fnumber;
  else
    (floatnum + i)->dnum = *(double *)&fnumber;
...
void setwordpost(ITOK *stok) /* for post word num setting */
{
  ...
  if (stok->post == USED_DIN_VAR) {
    ...
    (postbuf + posts)->type =
        (unsigned short)(am32 == FALSE ? POST_VAR : POST_VAR32);
  (postbuf + posts)->loc = outptr;
  posts++;
}
  ...
  if (tok == tk_number) {
    ...
    if (vop) { //%
      ...
      case 2:
        ...
        op66(razr);
        if (sign)
          outword(0xF8D1); // SAR AX,1
        else
          outword(0xE8D1); // SHR AX,1
        setzeroflag = TRUE;
        ClearReg(AX);
        break;
      default:
        ...
        if (vop != NUMNUM) {
          if (expand == TRUE) {
            if (chip > 2 || razr == r32) {
              ...
              op(vop); /* MOV CL,num */
              if (sign)
                outword(0xFad1); // SAR AX,1
              else
                outword(0xEad1);    // SHR AX,1
              outdword(0xfae2d8d1); // rcr ax,1  LOOP -6
              warningreg(begs[1]);
              setzeroflag = FALSE;
              ClearReg(AX);
            ...
            if (chip < 2 && razr == r16) {
              ...
              op(vop); /* MOV CL,num */
              if (sign)
                outword(0xF8D3); // SAR AX,CL
              else
                outword(0xE8D3); // SHR AX,CL
              warningreg(begs[1]);
              ClearReg(AX);
              ConstToReg(vop, CL, r8);
            } else {
              op66(razr);
              if (sign)
                outword(0xF8C1); // SAR AX,num
              else
                outword(0xE8C1); // SHR AX,num
              op(vop);
              ClearReg(AX);
            }
            setzeroflag = TRUE;
    ...
    case tk_wordvar:
      ...
      op(0xF7);
      if (sign)
        op(0x38 + itok.rm); /* IDIV word ptr [#] */
      else
        op(0x30 + itok.rm); /* DIV word ptr [#] */
      outaddress(&itok);
      ClearReg(AX);
      break;
    case tk_reg:
    ...
    defdiv:
      ...
      op66(razr);
      if (sign)
        outword(0xF9F7); /* IDIV CX */
      else
        outword(0xF1F7); /* DIV CX */
      next = 0;
      warningreg(regs[razr / 2 - 1][ECX]);
      ClearReg(CX);
      ClearReg(AX);
...
void DivNum(unsigned long num, int razr, int sign) {
  /*int i;
  unsigned long num2;
          if(num<65536&&optimizespeed&&(itok.flag&f_reloc)==0&&sign==0){
  //for signed needed new algoritm if(razr==r16&&chip>2){ num2=65536/(unsigned
  int)num+1; if((65535/num2)!=num)goto stddiv; op66(r32); op(0x25);
...
int getintoreg(int reg, int razr, int sign, char **ofsstr) {
  ...
  case tk_declare:
    break;
  default:
    //			if(cur_mod)break;	//10.08.04 22:50 из-за define
    //прекратить
    obuf = bufrm;
    bufrm = NULL;
    ostr = strinf;
    ...
    while ((!useeax) && itok.type != tp_stopper && tok != tk_eof) {
      ...
      if (itok.type == tp_opperand)
        ...
        case tk_modminus:
          if (j == 0)
            j = 1;
          if ((tok == tk_reg || tok == tk_reg32) && itok.number == reg)
            useeax = TRUE;
          if (j == 1) {
            if (tok == tk_number) {
              if (onlynum == FALSE && caselong(itok.number) == NUMNUM)
                j++;
            } else {
              j++;
              onlynum = FALSE;
...
void dobits() {
  ...
  if (tok2 == tk_assign) {
    ...
    if (tok == tk_number) {
      ...
      CheckAllMassiv(wbuf, razr, &wstr, &wtok);
      if (razr != r64)
        num2bits(&wtok, itok.number, razr);
      else {
        int siz = wtok.bit.siz;
        wtok.bit.siz = 32 - wtok.bit.ofs;
...
void bits2reg(int reg, int razr) {
  ...
  case r32:
    if (reg == AX) {
      getinto_e_ax(0, (razr == r16 ? tk_wordvar : tk_dwordvar), &wtok, wbuf,
                   &wstr, razr);
      if (razr == r16 && i != 16) {
...
void cpointr(int reg, int numpointr) {
  ...
  if (numpointr == itok.npointr) {
    ...
    if (itok.type >= tk_char && itok.type <= tk_float)
...
void cwpointr(ITOK *wtok, char *&wbuf, SINFO *wstr, int *otok, int npointr,
  ...
  if (wtok->type == tk_proc) {
    ...
    if (npointr == wtok->npointr) {
      ...
      if (wtok->type >= tk_char && wtok->type <= tk_float)
        *otok = tk_charvar + wtok->type - tk_char;
      else
...
int CheckAddOnly() {
  ...
  int j = 3;
  int changesign = 0;
  if (tok == tk_minusequals)
    changesign++;
  else if (tok != tk_plusequals)
    return FALSE;
  if (itok2.type == tp_stopper)
    return FALSE;
newloop:
  ...
  oinptr = inptr2;
  ocha = cha2;
  while (tok2 == tk_minus || tok2 == tk_mult)
    nexttok();
  while (itok.type != tp_stopper && tok != tk_eof) {
    nexttok();
    switch (tok) {
    ...
    if (itok.type == tp_opperand) {
      ...
      while (itok2.type == tp_opperand && tok != tk_eof)
        nexttok();
    } else {
  ...
  case tk_assign: //=
    ...
    if (itok2.type == tp_opperand) { //сложное выражение
      ...
      numbertovar:
        ...
        for (i = 0; i < 2; i++) {
          op66(r32);
          if (regoverstack && short_ok(itok.number, TRUE) &&
              (itok.flag & f_reloc) == 0) {
            op(0x6A);
            op(itok.number); // push short number
            op66(r32);
    ...
    if (itok2.type == tp_opperand) {
      ...
      num:
        for (i = 0; i < 2; i++) {
          ...
          if (tok != tk_apioffset && tok != tk_undefofs &&
              tok != tk_postnumber && (itok.flag & f_reloc) == 0 &&
      ...
      for (i = 0; i < 2; i++) {
        if (reg != (int)itok.number) {
          if (RegSwapReg(reg, itok.number, r32) == NOINREG) {
            ...
            waralreadinitreg(regs[1][reg], regs[1][itok.number]);
        }
        reg = r2;
        itok.number = vop / 256;
      }
  ...
  while (itok.type != tp_stopper && tok != tk_eof) {
    ...
    case tk_plus:
      ...
      case tk_reg:
        op66(r32);
        outword(0xB70F);
        if (itok.number == r1 || itok.number == r2)
          itok.number = reg1;
        op(0xC0 + itok.number * 9);
    ...
    case tk_rr:
      ...
      if (tok == tk_number) {
        ...
        if ((unsigned int)itok.number == 1) {
          op(0xD1);
          op(0xE8 + r2); // SHR reg,1
        } else if ((unsigned int)itok.number != 0) {
          op(0xc1);
          op(0xe8 + r2); // SHR reg,imm8
          op((unsigned int)itok.number);
        }
      } else if (r1 == ECX || r2 == ECX)
        regshifterror();
      else if ((tok == tk_beg || tok == tk_reg || tok == tk_reg32) &&
               itok.number == CL)
        goto rshift;
