...
void doposts() {
  ...
  if (comfile == file_bin)
    ...
    if ((outptrdata % 2) ==
        1) {      /* alignment of entire post data block manditory */
  ...
  for (i = 0; i < posts; i++) {
    ...
    case FIX_CODE_ADD:
      if (am32) {
        ...
        *(short *)&output[(postbuf + i)->loc] +=
            (short)(addval + (postbuf + i)->num);
...
int writeoutput() {
  ...
  if (comfile == file_com || comfile == file_sys || comfile == file_rom) {
    ...
    if (fwrite(output + startptr,
               comfile == file_rom ? romsize : outptr - startptr, 1,
               hout) != 1) {
      ErrWrite();
      return (-1);
    }
...
void MakeExeHeader(EXE_DOS_HEADER *exeheader) {
  ...
  if (modelmem == TINY && comfile == file_exe) {
    ...
    if (!resizemem) {
      ...
      exeheader->initSP =
          (unsigned short)(paragraphsrequired * 16); // intial value of SP
    ...
    exeheader->initSS =
        (unsigned short)((outptr + postsize + 15) / 16); // displacement of SS
    ...
    exeheader->initSP = (unsigned short)(outptrdata + postsize +
                                         stacksize); // intial value of SP
    exeheader->minmem = (unsigned short)paragraphsrequired; // min-paragraphs
    exeheader->maxmem = (unsigned short)paragraphsrequired; // max-paragraphs
  }
...
void warnunused(struct idrec *ptr) {
  ...
  if (ptr != NULL) {
    if (ptr->count == 0 && startupfile != ptr->file) {
      ...
      case tk_double:
        if (i <= 1 &&
            (ptr->recpost == DYNAMIC_VAR || ptr->recpost == DYNAMIC_POST))
          break;
...
void addinitvar() {
  ...
  if (numfloatconst) { //вставить константы float и привязать их
    ...
    for (i = 0; i < posts; i++) {
      if ((postbuf + i)->type == POST_FLOATNUM) {
        if (am32)
          *(unsigned long *)&output[(postbuf + i)->loc] =
              outptrdata + (postbuf + i)->num;
        else
          *(unsigned short *)&output[(postbuf + i)->loc] =
              (unsigned short)(outptrdata + (postbuf + i)->num);
        if (FixUp)
  ...
  for (i = 0; i < (unsigned int)numswtable;
       i++) { //создать и вставить таблицы switch
    int j;
    FSWI *swt = swtables + i;
    if (alignword)
      AlignCD(DS, swt->type);
    ...
    for (j = 0; j < swt->sizetab;
         j++) { //заполнить таблицу значениями по умолчанию
    ...
    for (int jj = 0; jj < swt->numcase; jj++) {
      ...
      if ((swt->info + jj)->type == singlcase) {
        if (swt->mode == 1) {
          ...
          if (am32 == FALSE)
            *(unsigned short *)&outputdata[oline + ii * 2] =
                (unsigned short)val;
          else
        ...
        for (; (unsigned int)j <= (swt->info + jj)->value; j++) {
          if (swt->mode == 1) {
            if (am32 == FALSE)
              *(unsigned short *)&outputdata[oline + j * 2] =
                  (unsigned short)val;
            else
              *(unsigned long *)&outputdata[oline + j * 4] = val;
          } else {
  ...
  for (i = 0; i < posts; i++) {
    if ((postbuf + i)->type == DIN_VAR || (postbuf + i)->type == DIN_VAR32) {
      ...
      if (FixUp)
        (postbuf + i)->type =
            (unsigned short)((postbuf + i)->type == DIN_VAR ? FIX_VAR
                                                            : FIX_VAR32);
      else
        killpost(i--);
    }
...
void setdindata(idrec *ptr, int i) {
  ...
  //*)&output[(postbuf+i)->loc],outptrdata,ptr->recnumber);
  if ((postbuf + i)->type == DIN_VAR)
    *(unsigned short *)&output[(postbuf + i)->loc] +=
        (unsigned short)(outptrdata);
  else
...
void AddUndefClassProc() {
  if (numundefclassproc == 0)
    ...
    undefclassproc = (idrec **)REALLOC(
        undefclassproc, sizeof(idrec **) * (numundefclassproc + 1));
